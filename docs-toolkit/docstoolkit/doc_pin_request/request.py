"""E383 DocPinRequest implementation.
...
@dataclass
class PinRequest:
    """A single pin request for a document."""
...
class DocPinRequest:
    """Main interface for managing pin requests."""
    ...
    def request_pin(
        ...
        """Create a new pin request in ``pending`` state."""
    ...
    def cancel(self, request_id: str) -> bool:
        """Cancel a pending request (alias for self-rejection without decider).

        Sets status to ``rejected`` with no ``decided_by``. Returns False if not
        pending or missing.
        """
        with self._lock:
            request = self._requests.get(request_id)
    ...
    def get(self, request_id: str) -> Optional[PinRequest]:
    ...
    def requests_by_user(
        ...
        """Return all requests by a given user, sorted by ``created_at``."""
        with self._lock:
            matched = [
                r for r in self._requests.values() if r.requested_by == user_id
            ]
        if status is not None:
            matched = [r for r in matched if r.status == status]
        return sorted(matched, key=lambda r: r.created_at)

    def pending_requests(self) -> list[PinRequest]:
    ...
    def stats(self) -> PinRequestStats:
