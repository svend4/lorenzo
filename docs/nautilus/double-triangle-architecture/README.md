---
state: normalized
---

# double-triangle-architecture
<!-- tags: readme, docs -->


<!-- summary -->
> README — раздел документации проекта Lorenzo. Кто ссылается на этот документ (16):


**Файлов:** 12

## Содержание

- [00-abstract.md](00-abstract.md) — <!-- toc -->
- [01-why-single-triangle-incomplete.md](01-why-single-triangle-incomplete.md) — <!-- toc-auto -->
- [02-double-triangle-architecture.md](02-double-triangle-architecture.md) — > [!IMPORTANT]
- [03-three-inter-layer-protocols.md](03-three-inter-layer-protocols.md) — > [!IMPORTANT]
- [04-nautilus-portal-substrate.md](04-nautilus-portal-substrate.md) — > [!NOTE]
- [05-pattern-library-bridge.md](05-pattern-library-bridge.md) — > [!TIP]
- [06-four-deployment-domains.md](06-four-deployment-domains.md) — > [!NOTE]
- [07-open-questions.md](07-open-questions.md) — > [!TIP]
- [08-call-to-action.md](08-call-to-action.md) — > [!TIP]
- [09-acknowledgments.md](09-acknowledgments.md) — <!-- toc-auto -->
- [10-references.md](10-references.md) — <!-- toc-auto -->
- [11-glossary.md](11-glossary.md) — > [!TIP]

<!-- backlinks -->

---

**Кто ссылается на этот документ (16):**
- [OUTLINE](../../OUTLINE.md)
- [READABILITY](../../READABILITY.md)
- [READING_TIME](../../READING_TIME.md)
- [SEARCH](../../SEARCH.md)
- [TABLES](../../TABLES.md)
- [00-abstract](00-abstract.md)
- [01-why-single-triangle-incomplete](01-why-single-triangle-incomplete.md)
- [03-three-inter-layer-protocols](03-three-inter-layer-protocols.md)
- _...ещё 8_


<!-- similar-docs -->

---

**Похожие документы:**
- [README](../../obsidian/nautilus/double-triangle-architecture/README.md) (сходство 0.91)
- [README](../../glossary/README.md) (сходство 0.33)
- [README](../representative-agent-layer-en/README.md) (сходство 0.33)



## Использование
```bash
# Запуск
python scripts/improve_readme.py
```
