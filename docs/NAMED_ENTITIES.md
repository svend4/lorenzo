# Индекс именованных сущностей

<!-- summary -->
> Всего сущностей: **183** (мин. 2 упоминаний)
**Проекты:** Svyazi, CardIndex, AgentFS, knowledge-space, LiteParse, Yodoca, MemNet, AutoResearch

---

<!-- toc -->
## Содержание

- [👤 People (19)](#people-19)
  - [`anthropic` (402 файлов)](#anthropic-402-файлов)
  - [`claude` (181 файлов)](#claude-181-файлов)
  - [`svend4` (102 файлов)](#svend4-102-файлов)
  - [`kksudo` (54 файлов)](#kksudo-54-файлов)
  - [`spbmolot` (50 файлов)](#spbmolot-50-файлов)
- [📦 Projects (97)](#projects-97)
  - [`nautilus` (187 файлов)](#nautilus-187-файлов)
  - [`svyazi` (126 файлов)](#svyazi-126-файлов)
  - [`GitHub` (120 файлов)](#github-120-файлов)
  - [`lorenzo` (94 файлов)](#lorenzo-94-файлов)
  - [`ngt` (83 файлов)](#ngt-83-файлов)
- [⚙️ Tech (31)](#tech-31)
  - [`mcp` (159 файлов)](#mcp-159-файлов)
  - [`api` (83 файлов)](#api-83-файлов)
  - [`rag` (75 файлов)](#rag-75-файлов)
  - [`llm` (73 файлов)](#llm-73-файлов)
  - [`json` (59 файлов)](#json-59-файлов)
- [🏢 Orgs (8)](#orgs-8)
  - [`вк` (133 файлов)](#вк-133-файлов)
  - [`meta` (98 файлов)](#meta-98-файлов)
  - [`mail` (69 файлов)](#mail-69-файлов)
  - [`openai` (45 файлов)](#openai-45-файлов)
  - [`google` (35 файлов)](#google-35-файлов)
- [📅 Dates (35)](#dates-35)
  - [`2026-04` (79 файлов)](#2026-04-79-файлов)
  - [`2026-04-29` (15 файлов)](#2026-04-29-15-файлов)
  - [`2026-04-19` (11 файлов)](#2026-04-19-11-файлов)
  - [`апрель 2026` (10 файлов)](#апрель-2026-10-файлов)
  - [`2026-04-26` (9 файлов)](#2026-04-26-9-файлов)

---

<!-- tags: memory, rag, knowledge, ingestion, local-first, architecture, roadmap, anthropic, self-improvement, collaboration -->




_Обновлено: 2026-04-29_

Всего сущностей: **183** (мин. 2 упоминаний)


## 👤 People (19)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `anthropic` | 402 | people |
| `claude` | 181 | people |
| `svend4` | 102 | people |
| `kksudo` | 54 | people |
| `spbmolot` | 50 | people |
| `anastasiyaw` | 30 | people |
| `vitalyoborin` | 28 | people |
| `andrey_chuyan` | 22 | people |
| `anthropics` | 3 | people |
| `yjs` | 3 | people |
| `artur-gavronchuk` | 3 | people |
| `nicholasspisak` | 3 | people |
| `vuguzum` | 3 | people |
| `dementev-dev` | 3 | people |
| `users` | 3 | people |
| `settings` | 2 | people |
| `kagvi13` | 2 | people |
| `ruvnet` | 2 | people |
| `lib4u` | 2 | people |

**Топ-5 по упоминаниям:**

### `anthropic` (402 файлов)

- `docs/01-svyazi/09-architectural-gaps.md`
- `docs/01-svyazi/11-integration-contracts.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- _...ещё 397_

### `claude` (181 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/08-conclusions.md`
- _...ещё 176_

### `svend4` (102 файлов)

- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/03-portal-protocol-md.md`
- _...ещё 97_

### `kksudo` (54 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/13-contacts.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/365-развёрнутый-анализ-внуковой-комбинации.md`
- _...ещё 49_

### `spbmolot` (50 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/13-contacts.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/QA.md`
- _...ещё 45_


## 📦 Projects (97)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `nautilus` | 187 | projects |
| `svyazi` | 126 | projects |
| `GitHub` | 120 | projects |
| `lorenzo` | 94 | projects |
| `ngt` | 83 | projects |
| `yodoca` | 78 | projects |
| `agentfs` | 75 | projects |
| `CardIndex` | 69 | projects |
| `knowledge-space` | 61 | projects |
| `obsidian` | 56 | projects |
| `LiteParse` | 54 | projects |
| `memnet` | 35 | projects |
| `notion` | 33 | projects |
| `PortalEntry` | 31 | projects |
| `AutoResearch` | 24 | projects |
| `VladSpace` | 23 | projects |
| `wikontic` | 22 | projects |
| `QueryResult` | 21 | projects |
| `gpt` | 19 | projects |
| `BaseAdapter` | 10 | projects |
| `ingit` | 10 | projects |
| `OpenWhispr` | 9 | projects |
| `TypeScript` | 9 | projects |
| `AutoGen` | 9 | projects |
| `mistral` | 9 | projects |
| `gemini` | 9 | projects |
| `CodeWiki` | 8 | projects |
| `langchain` | 8 | projects |
| `faiss` | 7 | projects |
| `DeepSeek` | 7 | projects |
| `DeepMind` | 7 | projects |
| `OpenClaw` | 7 | projects |
| `WebSocket` | 6 | projects |
| `ChatDev` | 6 | projects |
| `DevOps` | 6 | projects |
| `chromadb` | 6 | projects |
| `LlamaIndex` | 5 | projects |
| `pro2` | 5 | projects |
| `AutoAdapter` | 5 | projects |
| `HyperCortex` | 5 | projects |

**Топ-5 по упоминаниям:**

### `nautilus` (187 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/03-portal-protocol-md.md`
- `docs/02-anthropic-vacancies/04-abstract.md`
- _...ещё 182_

### `svyazi` (126 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- _...ещё 121_

### `GitHub` (120 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- _...ещё 115_

### `lorenzo` (94 файлов)

- `docs/01-svyazi/07-mvp-planning.md`
- `docs/02-anthropic-vacancies/12-content-overview.md`
- `docs/02-anthropic-vacancies/13-angle-perspective.md`
- `docs/02-anthropic-vacancies/134-the-double-triangle-architecture-md.md`
- `docs/02-anthropic-vacancies/151-open-knowledge-work-foundation-md.md`
- _...ещё 89_

### `ngt` (83 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/08-conclusions.md`
- _...ещё 78_


## ⚙️ Tech (31)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `mcp` | 159 | tech |
| `api` | 83 | tech |
| `rag` | 75 | tech |
| `llm` | 73 | tech |
| `json` | 59 | tech |
| `markdown` | 54 | tech |
| `git` | 46 | tech |
| `python` | 44 | tech |
| `yaml` | 43 | tech |
| `go` | 39 | tech |
| `rest` | 31 | tech |
| `html` | 15 | tech |
| `transformer` | 13 | tech |
| `ci` | 13 | tech |
| `sqlite` | 12 | tech |
| `cd` | 10 | tech |
| `vector` | 9 | tech |
| `react` | 8 | tech |
| `docker` | 7 | tech |
| `rust` | 7 | tech |
| `postgresql` | 6 | tech |
| `jaccard` | 5 | tech |
| `fastapi` | 4 | tech |
| `bm25` | 4 | tech |
| `cosine` | 3 | tech |
| `css` | 2 | tech |
| `graphql` | 2 | tech |
| `webhook` | 2 | tech |
| `kubernetes` | 2 | tech |

**Топ-5 по упоминаниям:**

### `mcp` (159 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- _...ещё 154_

### `api` (83 файлов)

- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/09-architectural-gaps.md`
- _...ещё 78_

### `rag` (75 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- _...ещё 70_

### `llm` (73 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- _...ещё 68_

### `json` (59 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/03-portal-protocol-md.md`
- `docs/02-anthropic-vacancies/07-2-terminology.md`
- `docs/02-anthropic-vacancies/08-3-registry-nautilus-json.md`
- _...ещё 54_


## 🏢 Orgs (8)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `вк` | 133 | orgs |
| `meta` | 98 | orgs |
| `mail` | 33 | orgs |
| `openai` | 21 | orgs |
| `google` | 14 | orgs |
| `microsoft` | 11 | orgs |
| `yandex` | 5 | orgs |
| `сбер` | 3 | orgs |

**Топ-5 по упоминаниям:**

### `вк` (133 файлов)

- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/12-roadmap.md`
- `docs/01-svyazi/13-contacts.md`
- _...ещё 128_

### `meta` (98 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/08-3-registry-nautilus-json.md`
- _...ещё 93_

### `mail` (69 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/09-architectural-gaps.md`
- _...ещё 64_

### `openai` (45 файлов)

- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/150-appendix-c-version-history.md`
- `docs/02-anthropic-vacancies/155-1-problem-statement.md`
- _...ещё 40_

### `google` (35 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/138-1-why-single-triangle-models-are-incomplete.md`
- `docs/02-anthropic-vacancies/150-appendix-c-version-history.md`
- `docs/02-anthropic-vacancies/155-1-problem-statement.md`
- _...ещё 30_


## 📅 Dates (35)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `2026-04` | 79 | dates |
| `2026-04-29` | 15 | dates |
| `2026-04-19` | 11 | dates |
| `апрель 2026` | 10 | dates |
| `2026-04-26` | 9 | dates |
| `апреля 2026` | 7 | dates |
| `в 2026 году` | 6 | dates |
| `2026/04/25` | 6 | dates |
| `март 2026` | 5 | dates |
| `декабрь 2025` | 5 | dates |
| `декабря 2025` | 4 | dates |
| `2025-12-15` | 4 | dates |
| `2024-01-01` | 4 | dates |
| `феврале 2025` | 4 | dates |
| `май 2025` | 4 | dates |
| `2026-04-15` | 4 | dates |
| `Сентябрь 2025` | 4 | dates |
| `январе 2026` | 4 | dates |
| `марта 2026` | 3 | dates |
| `декабрь 2024` | 3 | dates |
| `2026-05-03` | 3 | dates |
| `2025-12-15` | 3 | dates |
| `2024-01-01` | 3 | dates |
| `феврале 2025` | 3 | dates |
| `2026-04-15` | 3 | dates |
| `май 2025` | 3 | dates |
| `Сентябрь 2025` | 3 | dates |
| `января 2026` | 3 | dates |
| `февраль 2026` | 2 | dates |
| `2026-04-22` | 2 | dates |
| `ноябре 2025` | 2 | dates |
| `2025-11-12` | 2 | dates |
| `2026-02-01` | 2 | dates |
| `февраля 2026` | 2 | dates |
| `2026-10-15` | 2 | dates |

**Топ-5 по упоминаниям:**

### `2026-04` (79 файлов)

- `docs/02-anthropic-vacancies/03-portal-protocol-md.md`
- `docs/02-anthropic-vacancies/103-appendix-b-change-log.md`
- `docs/02-anthropic-vacancies/105-review-methodology-md.md`
- `docs/02-anthropic-vacancies/121-appendix-c-история-изменений-методологии.md`
- `docs/02-anthropic-vacancies/135-a-formal-model-for-human-ai-collaboration-in-distr.md`
- _...ещё 74_

### `2026-04-29` (15 файлов)

- `docs/04-ai-collaborations/01-executive-summary.md`
- `docs/04-ai-collaborations/02-методика-и-рамка-отбора.md`
- `docs/04-ai-collaborations/07-выводы.md`
- `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
- `docs/04-ai-collaborations/10-новые-ансамбли-следующего-шага.md`
- _...ещё 10_

### `2026-04-19` (11 файлов)

- `docs/02-anthropic-vacancies/103-appendix-b-change-log.md`
- `docs/02-anthropic-vacancies/133-обратная-связь.md`
- `docs/02-anthropic-vacancies/135-a-formal-model-for-human-ai-collaboration-in-distr.md`
- `docs/02-anthropic-vacancies/150-appendix-c-version-history.md`
- `docs/02-anthropic-vacancies/152-ai-coordinated-infrastructure-for-distributed-expe.md`
- _...ещё 6_

### `апрель 2026` (10 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/114-7-реализация-в-проекте-nautilus.md`
- `docs/02-anthropic-vacancies/234-3-эмпирический-кейс-обучай.md`
- `docs/02-anthropic-vacancies/341-приложение-c-образец-спецификаций-инструментов-ing.md`
- _...ещё 5_

### `2026-04-26` (9 файлов)

- `docs/02-anthropic-vacancies/209-a-typology-of-ai-agents-on-the-principal-side-and-.md`
- `docs/02-anthropic-vacancies/229-профессиональные-коллеги-агенты.md`
- `docs/02-anthropic-vacancies/251-ai-support-through-configurable-specialist-ensembl.md`
- `docs/02-anthropic-vacancies/274-the-missing-middle-layer-between-chat-and-code.md`
- `docs/02-anthropic-vacancies/289-инфраструктура-для-ai-совместной-интеллектуальной-.md`
- _...ещё 4_

