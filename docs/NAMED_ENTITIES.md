# Индекс именованных сущностей

<!-- toc -->
## Содержание

- [Contents](#contents)
- [👤 People (20)](#people-20)
  - [`anthropic` (654 файлов)](#anthropic-654-файлов)
  - [`claude` (453 файлов)](#claude-453-файлов)
  - [`svend4` (192 файлов)](#svend4-192-файлов)
  - [`kksudo` (77 файлов)](#kksudo-77-файлов)
  - [`spbmolot` (71 файлов)](#spbmolot-71-файлов)
- [📦 Projects (150)](#projects-150)
  - [`lorenzo` (516 файлов)](#lorenzo-516-файлов)
  - [`nautilus` (404 файлов)](#nautilus-404-файлов)
  - [`svyazi` (390 файлов)](#svyazi-390-файлов)
  - [`GitHub` (262 файлов)](#github-262-файлов)
  - [`yodoca` (160 файлов)](#yodoca-160-файлов)
- [⚙️ Tech (31)](#tech-31)
  - [`mcp` (357 файлов)](#mcp-357-файлов)
  - [`bm25` (257 файлов)](#bm25-257-файлов)
  - [`llm` (216 файлов)](#llm-216-файлов)
  - [`api` (182 файлов)](#api-182-файлов)
  - [`rag` (168 файлов)](#rag-168-файлов)
- [🏢 Orgs (8)](#orgs-8)
  - [`вк` (298 файлов)](#вк-298-файлов)
  - [`meta` (218 файлов)](#meta-218-файлов)
  - [`mail` (72 файлов)](#mail-72-файлов)
  - [`openai` (47 файлов)](#openai-47-файлов)
  - [`google` (38 файлов)](#google-38-файлов)
- [📅 Dates (39)](#dates-39)
  - [`2026-04` (83 файлов)](#2026-04-83-файлов)
  - [`2026-05` (58 файлов)](#2026-05-58-файлов)
  - [`2026-04-29` (43 файлов)](#2026-04-29-43-файлов)
  - [`2026-04-19` (18 файлов)](#2026-04-19-18-файлов)
  - [`апрель 2026` (17 файлов)](#апрель-2026-17-файлов)
- [Использование](#использование)
- [Смотрите также](#смотрите-также)

---


<!-- toc-auto -->
## Contents

- [👤 People (20)](#people-20)
  - [anthropic (654 файлов)](#anthropic-654-файлов)
  - [claude (453 файлов)](#claude-453-файлов)
  - [svend4 (192 файлов)](#svend4-192-файлов)
  - [kksudo (77 файлов)](#kksudo-77-файлов)
  - [spbmolot (71 файлов)](#spbmolot-71-файлов)
- [📦 Projects (150)](#projects-150)
  - [lorenzo (516 файлов)](#lorenzo-516-файлов)
  - [nautilus (404 файлов)](#nautilus-404-файлов)
  - [svyazi (390 файлов)](#svyazi-390-файлов)
  - [GitHub (262 файлов)](#github-262-файлов)
  - [yodoca (160 файлов)](#yodoca-160-файлов)
- [⚙️ Tech (31)](#tech-31)
  - [mcp (357 файлов)](#mcp-357-файлов)
  - [bm25 (257 файлов)](#bm25-257-файлов)
  - [llm (216 файлов)](#llm-216-файлов)
  - [api (182 файлов)](#api-182-файлов)
  - [rag (168 файлов)](#rag-168-файлов)
- [🏢 Orgs (8)](#orgs-8)
  - [вк (298 файлов)](#вк-298-файлов)
  - [meta (218 файлов)](#meta-218-файлов)
  - [mail (72 файлов)](#mail-72-файлов)
  - [openai (47 файлов)](#openai-47-файлов)
  - [google (38 файлов)](#google-38-файлов)
- [📅 Dates (39)](#dates-39)
  - [2026-04 (83 файлов)](#2026-04-83-файлов)
  - [2026-05 (58 файлов)](#2026-05-58-файлов)
  - [2026-04-29 (43 файлов)](#2026-04-29-43-файлов)
  - [2026-04-19 (18 файлов)](#2026-04-19-18-файлов)
  - [апрель 2026 (17 файлов)](#апрель-2026-17-файлов)
- [Использование](#использование)
- [Смотрите также](#смотрите-также)



> [!NOTE]
> Раздел `NAMED_ENTITIES` формируется автоматически из данных репозитория.

<!-- alert-added -->
<!-- tags: named-entities, docs -->


<!-- summary -->
> `NAMED_ENTITIES` — раздел документации проекта Lorenzo.


_Обновлено: 2026-05-11_

Всего сущностей: **248** (мин. 2 упоминаний)


## 👤 People (20)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `anthropic` | 654 | people |
| `claude` | 453 | people |
| `svend4` | 192 | people |
| `kksudo` | 77 | people |
| `spbmolot` | 71 | people |
| `anastasiyaw` | 48 | people |
| `vitalyoborin` | 39 | people |
| `andrey_chuyan` | 14 | people |
| `vuguzum` | 6 | people |
| `anthropics` | 5 | people |
| `dementev-dev` | 5 | people |
| `artur-gavronchuk` | 5 | people |
| `settings` | 4 | people |
| `nicholasspisak` | 4 | people |
| `yjs` | 4 | people |
| `users` | 4 | people |
| `camel-ai` | 4 | people |
| `kagvi13` | 3 | people |
| `lib4u` | 2 | people |
| `ruvnet` | 2 | people |

**Топ-5 по упоминаниям:**

### `anthropic` (654 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/01-svyazi/09-architectural-gaps.md`
- `docs/01-svyazi/11-integration-contracts.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- _...ещё 649_

### `claude` (453 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/08-conclusions.md`
- _...ещё 448_

### `svend4` (192 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/05-0-status-of-this-document.md`
- `docs/02-anthropic-vacancies/07-2-terminology.md`
- _...ещё 187_

### `kksudo` (77 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/13-contacts.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/365-развёрнутый-анализ-внуковой-комбинации.md`
- _...ещё 72_

### `spbmolot` (71 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/13-contacts.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/QA.md`
- _...ещё 66_


## 📦 Projects (150)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `lorenzo` | 516 | projects |
| `nautilus` | 404 | projects |
| `svyazi` | 390 | projects |
| `GitHub` | 262 | projects |
| `yodoca` | 160 | projects |
| `ngt` | 142 | projects |
| `CardIndex` | 132 | projects |
| `agentfs` | 119 | projects |
| `knowledge-space` | 113 | projects |
| `memnet` | 106 | projects |
| `obsidian` | 104 | projects |
| `LiteParse` | 78 | projects |
| `notion` | 65 | projects |
| `wikontic` | 53 | projects |
| `PortalEntry` | 51 | projects |
| `AutoResearch` | 49 | projects |
| `gpt` | 42 | projects |
| `AutoGen` | 22 | projects |
| `gemini` | 22 | projects |
| `OpenClaw` | 22 | projects |
| `QueryResult` | 21 | projects |
| `ClickHouse` | 19 | projects |
| `LangChain` | 18 | projects |
| `OpenWhispr` | 17 | projects |
| `CodeWiki` | 17 | projects |
| `BaseAdapter` | 17 | projects |
| `CardStore` | 17 | projects |
| `VladSpace` | 16 | projects |
| `DeepSeek` | 16 | projects |
| `LlamaIndex` | 15 | projects |
| `VitaliySemenov` | 15 | projects |
| `ChatDev` | 15 | projects |
| `LangGraph` | 15 | projects |
| `confluence` | 15 | projects |
| `faiss` | 14 | projects |
| `OpenClaude` | 14 | projects |
| `TypeScript` | 13 | projects |
| `mistral` | 12 | projects |
| `chromadb` | 12 | projects |
| `BrainBox` | 12 | projects |

**Топ-5 по упоминаниям:**

### `lorenzo` (516 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/01-svyazi/QA.md`
- `docs/02-anthropic-vacancies/102-доступ-к-данным.md`
- `docs/02-anthropic-vacancies/113-6-почему-это-валидный-паттерн-для-ai-assisted-work.md`
- `docs/02-anthropic-vacancies/118-appendix-a-шаблон-для-header-warning.md`
- _...ещё 511_

### `nautilus` (404 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/04-abstract.md`
- `docs/02-anthropic-vacancies/05-0-status-of-this-document.md`
- _...ещё 399_

### `svyazi` (390 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- _...ещё 385_

### `GitHub` (262 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- _...ещё 257_

### `yodoca` (160 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/08-conclusions.md`
- _...ещё 155_


## ⚙️ Tech (31)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `mcp` | 357 | tech |
| `bm25` | 257 | tech |
| `llm` | 216 | tech |
| `api` | 182 | tech |
| `rag` | 168 | tech |
| `python` | 132 | tech |
| `yaml` | 98 | tech |
| `markdown` | 97 | tech |
| `json` | 87 | tech |
| `git` | 73 | tech |
| `go` | 67 | tech |
| `rest` | 55 | tech |
| `sqlite` | 43 | tech |
| `html` | 40 | tech |
| `ci` | 31 | tech |
| `cd` | 27 | tech |
| `transformer` | 24 | tech |
| `vector` | 22 | tech |
| `postgresql` | 21 | tech |
| `react` | 17 | tech |
| `sql` | 17 | tech |
| `docker` | 15 | tech |
| `cosine` | 12 | tech |
| `rust` | 10 | tech |
| `jaccard` | 9 | tech |
| `css` | 6 | tech |
| `webhook` | 5 | tech |
| `kubernetes` | 5 | tech |
| `terraform` | 4 | tech |
| `graphql` | 3 | tech |
| `fastapi` | 3 | tech |

**Топ-5 по упоминаниям:**

### `mcp` (357 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- _...ещё 352_

### `bm25` (257 файлов)

- `docs/01-svyazi/00-intro-part2.md`
- `docs/02-anthropic-vacancies/118-appendix-a-шаблон-для-header-warning.md`
- `docs/02-anthropic-vacancies/120-главные-технические-риски.md`
- `docs/02-anthropic-vacancies/126-установка.md`
- `docs/02-anthropic-vacancies/131-ограничения-текущей-версии-0-1-0-draft.md`
- _...ещё 252_

### `llm` (216 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- _...ещё 211_

### `api` (182 файлов)

- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/09-architectural-gaps.md`
- _...ещё 177_

### `rag` (168 файлов)

- `docs/01-svyazi/01-executive-summary.md`
- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/07-mvp-planning.md`
- _...ещё 163_


## 🏢 Orgs (8)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `вк` | 298 | orgs |
| `meta` | 218 | orgs |
| `mail` | 72 | orgs |
| `openai` | 47 | orgs |
| `google` | 38 | orgs |
| `microsoft` | 20 | orgs |
| `yandex` | 15 | orgs |
| `сбер` | 8 | orgs |

**Топ-5 по упоминаниям:**

### `вк` (298 файлов)

- `docs/01-svyazi/02-methodology.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/12-roadmap.md`
- `docs/01-svyazi/13-contacts.md`
- _...ещё 293_

### `meta` (218 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/02-общий-план-развития-nautilus-portal-protocol.md`
- `docs/02-anthropic-vacancies/08-3-registry-nautilus-json.md`
- _...ещё 213_

### `mail` (72 файлов)

- `docs/01-svyazi/03-component-catalog.md`
- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/01-svyazi/06-security-privacy.md`
- `docs/01-svyazi/07-mvp-planning.md`
- `docs/01-svyazi/09-architectural-gaps.md`
- _...ещё 67_

### `openai` (47 файлов)

- `docs/01-svyazi/04-ensembles-overview.md`
- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/155-1-problem-statement.md`
- `docs/02-anthropic-vacancies/163-9-call-for-partnership.md`
- _...ещё 42_

### `google` (38 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/138-1-why-single-triangle-models-are-incomplete.md`
- `docs/02-anthropic-vacancies/155-1-problem-statement.md`
- `docs/02-anthropic-vacancies/163-9-call-for-partnership.md`
- _...ещё 33_


## 📅 Dates (39)

| Сущность | Файлов | Тип |
|----------|--------|-----|
| `2026-04` | 83 | dates |
| `2026-05` | 58 | dates |
| `2026-04-29` | 43 | dates |
| `2026-04-19` | 18 | dates |
| `апрель 2026` | 17 | dates |
| `2026-05-10` | 12 | dates |
| `2026-05-11` | 12 | dates |
| `апреля 2026` | 11 | dates |
| `2026/04/25` | 9 | dates |
| `2026-04-26` | 8 | dates |
| `в 2026 году` | 7 | dates |
| `марта 2026` | 6 | dates |
| `март 2026` | 6 | dates |
| `апреле 2026` | 6 | dates |
| `2024-06-15` | 5 | dates |
| `декабрь 2025` | 4 | dates |
| `2026-05-03` | 4 | dates |
| `2025-12-15` | 4 | dates |
| `2024-01-01` | 4 | dates |
| `Сентябрь 2025` | 4 | dates |
| `2026-04-22` | 4 | dates |
| `декабря 2025` | 3 | dates |
| `2024-01` | 3 | dates |
| `2026-04-15` | 3 | dates |
| `января 2026` | 3 | dates |
| `февраль 2026` | 3 | dates |
| `2026-05-04` | 3 | dates |
| `2024-03-15` | 3 | dates |
| `феврале 2025` | 3 | dates |
| `Q1 2026` | 3 | dates |
| `февраля 2026` | 3 | dates |
| `май 2025` | 3 | dates |
| `декабрь 2024` | 2 | dates |
| `январе 2026` | 2 | dates |
| `март 2024` | 2 | dates |
| `Q2 2024` | 2 | dates |
| `май 2026` | 2 | dates |
| `2025-11-12` | 2 | dates |
| `2020-01-01` | 2 | dates |

**Топ-5 по упоминаниям:**

### `2026-04` (83 файлов)

- `docs/02-anthropic-vacancies/103-appendix-b-change-log.md`
- `docs/02-anthropic-vacancies/121-appendix-c-история-изменений-методологии.md`
- `docs/02-anthropic-vacancies/135-a-formal-model-for-human-ai-collaboration-in-distr.md`
- `docs/02-anthropic-vacancies/141-4-nautilus-portal-as-reference-substrate.md`
- `docs/02-anthropic-vacancies/152-ai-coordinated-infrastructure-for-distributed-expe.md`
- _...ещё 78_

### `2026-05` (58 файлов)

- `docs/05-habr-projects/knowledge/agentfs.md`
- `docs/05-habr-projects/knowledge/knowledge-space.md`
- `docs/05-habr-projects/knowledge/mclaude.md`
- `docs/05-habr-projects/knowledge/research-docs-liteparse.md`
- `docs/05-habr-projects/knowledge/rufler.md`
- _...ещё 53_

### `2026-04-29` (43 файлов)

- `docs/04-ai-collaborations/01-executive-summary.md`
- `docs/04-ai-collaborations/02-методика-и-рамка-отбора.md`
- `docs/04-ai-collaborations/07-выводы.md`
- `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
- `docs/04-ai-collaborations/10-новые-ансамбли-следующего-шага.md`
- _...ещё 38_

### `2026-04-19` (18 файлов)

- `docs/02-anthropic-vacancies/103-appendix-b-change-log.md`
- `docs/02-anthropic-vacancies/133-обратная-связь.md`
- `docs/02-anthropic-vacancies/135-a-formal-model-for-human-ai-collaboration-in-distr.md`
- `docs/02-anthropic-vacancies/150-appendix-c-version-history.md`
- `docs/02-anthropic-vacancies/152-ai-coordinated-infrastructure-for-distributed-expe.md`
- _...ещё 13_

### `апрель 2026` (17 файлов)

- `docs/02-anthropic-vacancies/00-intro.md`
- `docs/02-anthropic-vacancies/01-интегральный-анализ-профиля-svend4.md`
- `docs/02-anthropic-vacancies/114-7-реализация-в-проекте-nautilus.md`
- `docs/02-anthropic-vacancies/234-3-эмпирический-кейс-обучай.md`
- `docs/02-anthropic-vacancies/341-приложение-c-образец-спецификаций-инструментов-ing.md`
- _...ещё 12_



## Использование
```bash
# Запуск
python scripts/improve_named_entities.py
```

## Смотрите также
- [Главная](README.md)
- [Метрики](METRICS.md)
- [Здоровье](HEALTH.md)
- [Глоссарий](GLOSSARY.md)
- [Сущности](ENTITIES.md)
- [Решения](DECISIONS.md)

<!-- backlinks -->

---

**Кто ссылается на этот документ (9):**
- [CODE_BLOCKS](CODE_BLOCKS.md)
- [DIGEST_AUTO](DIGEST_AUTO.md)
- [LINK_PREVIEW](LINK_PREVIEW.md)
- [OUTLINE](OUTLINE.md)
- [READABILITY](READABILITY.md)
- [READING_TIME](READING_TIME.md)
- [README](README.md)
- [SEARCH](SEARCH.md)
- _...ещё 1_

