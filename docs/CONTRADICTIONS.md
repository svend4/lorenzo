# Противоречия в базе знаний

_Обновлено: 2026-04-29_

Утверждений: **42228** | Противоречий: **3683**

> Автоматический поиск без LLM — возможны ложные срабатывания.

## Найденные противоречия

### 1. 🔢 Числовое — 42.0 vs 81.0 (уверенность: 0.8)

**Общие ключевые слова:** `docs`, `liteparse`, `nlaik`, `research`

**A:** `docs/contacts/antipozitive.md`
> md) 42% - Контакт: nlaik / LiteParse / research-docs(nlaik

**B:** `docs/contacts/sonia-black.md`
> md) 81% - Контакт: nlaik / LiteParse / research-docs(nlaik

---

### 2. 🔢 Числовое — 42.0 vs 66.0 (уверенность: 0.8)

**Общие ключевые слова:** `автор`, `вовлечён`, `контекст`, `непосредственно`

**A:** `docs/02-anthropic-vacancies/239-8-пилотное-предложение-sgb-колega-адвокат.md`
> Контекст Автор этой статьи непосредственно вовлечён в немецкое социальное право через текущие разбирательства в Sozialgericht (дела S 6 SO 58/26 ER и 

**B:** `docs/nautilus/professional-colleague-agents-ru/08-pilot-sgb-kolega.md`
> Контекст Автор этой статьи непосредственно вовлечён в немецкое социальное право через текущие разбирательства в Sozialgericht (дела S 6 SO 58/26 ER и 

---

### 3. 🔢 Числовое — 1017200.0 vs 44.0 (уверенность: 0.8)

**Общие ключевые слова:** `годы`, `деятельность`, `добавить`, `областей`

**A:** `docs/02-anthropic-vacancies/199-9-стратегия-поэтапного-развёртывания.md`
> Фаза 2 — Расширение Областей (Годы 3-4) Деятельность: - Добавить области 2 (профессионалы на пенсии) и 8 (студенты) — обе относительно низкорисковые р

**B:** `docs/nautilus/representative-agent-layer-ru/09-strategiya-razvyortyvaniya.md`
> Фаза 2 — Расширение Областей (Годы 3-4) Деятельность: - Добавить области 2 (профессионалы на пенсии) и 8 (студенты) — обе относительно низкорисковые р

---

### 7. 🔢 Числовое — 68.0 vs 8.0 (уверенность: 0.8)

**Общие ключевые слова:** `collaborations`, `docs`, `добавляет`, `продолжение`

**A:** `docs/01-svyazi/01-executive-summary.md`
> 68) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

**B:** `docs/01-svyazi/08-conclusions.md`
> md) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

---

### 8. 🔢 Числовое — 68.0 vs 4.0 (уверенность: 0.8)

**Общие ключевые слова:** `collaborations`, `docs`, `добавляет`, `продолжение`

**A:** `docs/01-svyazi/01-executive-summary.md`
> 68) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

**B:** `docs/VALIDATION.md`
> md: docs/04-ai-collaborations/08-что-это-продолжение-добавляет

---

### 9. 🔢 Числовое — 8.0 vs 4.0 (уверенность: 0.8)

**Общие ключевые слова:** `collaborations`, `docs`, `добавляет`, `продолжение`

**A:** `docs/01-svyazi/08-conclusions.md`
> md) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

**B:** `docs/04-ai-collaborations/07-выводы.md`
> md) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

---

### 10. 🔢 Числовое — 8.0 vs 4.0 (уверенность: 0.8)

**Общие ключевые слова:** `collaborations`, `docs`, `добавляет`, `продолжение`

**A:** `docs/01-svyazi/08-conclusions.md`
> md) - 08-что-это-продолжение-добавляет(docs/04-ai-collaborations/08-что-это-продолжение-добавляет

**B:** `docs/VALIDATION.md`
> md: docs/04-ai-collaborations/08-что-это-продолжение-добавляет

---

### 11. 🔢 Числовое — 8.0 vs 58.0 (уверенность: 0.8)

**Общие ключевые слова:** `добавляет`, `продолжение`

**A:** `docs/04-ai-collaborations/README.md`
> md(08-что-это-продолжение-добавляет

**B:** `docs/LANGUAGE_STATS.md`
> md 58% 42% 08-что-это-продолжение-добавляет

---

### 12. 🔢 Числовое — 24.0 vs 7.0 (уверенность: 0.8)

**Общие ключевые слова:** `habr`, `moltbot`, `openclaw`, `автономного`

**A:** `docs/04-ai-collaborations/00-intro.md`
> Habr Moltbot/OpenClaw добавляет автономного агента 24/7 с инструментами, браузером, почтой, API, фоном и 700+ skills; автор подчёркивает отличие от Ch

**B:** `docs/ai-collaborations/ensembles/7-domain-agent-app-factory.md`
> Habr Moltbot/OpenClaw добавляет автономного агента 24/7 с инструментами, браузером, почтой, API, фоном и 700+ skills; автор подчёркивает отличие от Ch

---

### 4. 🔢 Числовое — 8.0 vs 44.0 (уверенность: 0.8)

**Общие ключевые слова:** `sequential`, `вместо`, `если`, `одного`

**A:** `docs/04-ai-collaborations/00-intro.md`
> Если поставить вместо одного человека Sequential-протокол из 8 малых агентов на дешёвых моделях, каждый видит результаты предшественников, — получаешь

**B:** `docs/05-habr-projects/memory/memnet.md`
> Если поставить вместо одного человека Sequential-протокол из 8 малых агентов на дешёвых моделях, каждый видит результаты предшественников, — получаешь

---

### 5. 🔢 Числовое — 70.0 vs 4.0 (уверенность: 0.8)

**Общие ключевые слова:** `второе`, `месячная`, `одна`, `отрезвляющее`

**A:** `docs/02-anthropic-vacancies/00-intro.md`
> Второе — отрезвляющее: эти 70 репо — это одна 4-месячная брейн-волна , а не плоды многолетней разработки

**B:** `docs/CONCEPTS.md`
> md) Второе — отрезвляющее: эти 70 репо : одна 4-месячная брейн-волна , а не плоды многолетней разработки → 00-intro(docs/02-anthropic-vacancies/00-int

---

### 6. 🔢 Числовое — 22.0 vs 4.0 (уверенность: 0.8)

**Общие ключевые слова:** `context`, `mcp`, `model`, `protocol`

**A:** `docs/01-svyazi/01-executive-summary.md`
> 22) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

**B:** `docs/01-svyazi/03-component-catalog.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

---

### 7. 🔢 Числовое — 22.0 vs 3.0 (уверенность: 0.8)

**Общие ключевые слова:** `context`, `mcp`, `model`, `protocol`

**A:** `docs/01-svyazi/01-executive-summary.md`
> 22) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

**B:** `docs/01-svyazi/06-security-privacy.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^llm: Large Language Model — большая языковая модель ^pii: Personally Identifiable

---

### 8. 🔢 Числовое — 4.0 vs 785.0 (уверенность: 0.8)

**Общие ключевые слова:** `context`, `mcp`, `model`, `protocol`

**A:** `docs/01-svyazi/03-component-catalog.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

**B:** `docs/04-ai-collaborations/03-карта-найденных-проектов-и-паттернов.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

---

### 9. 🔢 Числовое — 4.0 vs 6.0 (уверенность: 0.8)

**Общие ключевые слова:** `context`, `mcp`, `model`, `protocol`

**A:** `docs/01-svyazi/03-component-catalog.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^rag: Retrieval-Augmented Generation — генерация с поиском ^llm: Large Language Mo

**B:** `docs/04-ai-collaborations/06-безопасность-приватность-и-бюджетный-роутинг.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^llm: Large Language Model — большая языковая модель ^pii: Personally Identifiable

---

### 10. 🔢 Числовое — 3.0 vs 6.0 (уверенность: 0.8)

**Общие ключевые слова:** `context`, `mcp`, `model`, `protocol`

**A:** `docs/01-svyazi/06-security-privacy.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^llm: Large Language Model — большая языковая модель ^pii: Personally Identifiable

**B:** `docs/04-ai-collaborations/06-безопасность-приватность-и-бюджетный-роутинг.md`
> md) --- ^mcp: Model Context Protocol — протокол для AI-инструментов ^llm: Large Language Model — большая языковая модель ^pii: Personally Identifiable

---

### 11. 🔢 Числовое — 5.0 vs 30.0 (уверенность: 0.8)

**Общие ключевые слова:** `edge`, `mcp`, `skills`, `tinyml`

**A:** `docs/04-ai-collaborations/00-intro.md`
> TinyML/Edge AI × MCP-протокол + skills-система Родители: edge-устройства уровня Jetson Orin / Raspberry Pi 5 / Coral с TinyChat от MIT HAN Lab (LLaMA-

**B:** `docs/05-habr-projects/memory/memnet.md`
> TinyML/Edge AI × MCP-протокол + skills-система Родители: edge-устройства уровня Jetson Orin / Raspberry Pi 5 / Coral с TinyChat от MIT HAN Lab (LLaMA-

---

### 12. 🔢 Числовое — 8.0 vs 29.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/01-svyazi/01-executive-summary.md`
> md) Связанные документы - 08 Conclusions(08-conclusions

**B:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) 29% Связанные документы - 08 Conclusions(

---

### 13. 🔢 Числовое — 8.0 vs 21.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/01-svyazi/01-executive-summary.md`
> md) Связанные документы - 08 Conclusions(08-conclusions

**B:** `docs/04-ai-collaborations/07-выводы.md`
> md) 21% Связанные документы - 08 Conclusions(

---

### 14. 🔢 Числовое — 8.0 vs 17.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/01-svyazi/01-executive-summary.md`
> md) Связанные документы - 08 Conclusions(08-conclusions

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 17% Связанные документы - 08 Conclusions(

---

### 15. 🔢 Числовое — 8.0 vs 21.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/07-выводы.md`
> md) 21% Связанные документы - 08 Conclusions(

---

### 16. 🔢 Числовое — 8.0 vs 17.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 17% Связанные документы - 08 Conclusions(

---

### 17. 🔢 Числовое — 29.0 vs 8.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) 29% Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/07-выводы.md`
> md) Связанные документы - 08 Conclusions(

---

### 18. 🔢 Числовое — 29.0 vs 8.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) 29% Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/07-выводы.md`
> md) 21% Связанные документы - 08 Conclusions(

---

### 19. 🔢 Числовое — 29.0 vs 17.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/01-executive-summary.md`
> md) 29% Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 17% Связанные документы - 08 Conclusions(

---

### 20. 🔢 Числовое — 21.0 vs 8.0 (уверенность: 0.8)

**Общие ключевые слова:** `conclusions`, `документы`, `связанные`

**A:** `docs/04-ai-collaborations/07-выводы.md`
> md) 21% Связанные документы - 08 Conclusions(

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 17% Связанные документы - 08 Conclusions(

---

### 21. 🔢 Числовое — 33.0 vs 21.0 (уверенность: 0.8)

**Общие ключевые слова:** `knowledge`, `wikontic`, `граф`, `семантический`

**A:** `docs/05-habr-projects/01-synthesis.md`
> md) 33% - Wikontic: семантический граф(knowledge/wikontic

**B:** `docs/05-habr-projects/02-collaboration-partners.md`
> md) 21% - Wikontic: семантический граф(knowledge/wikontic

---

### 22. 🔢 Числовое — 29.0 vs 5.0 (уверенность: 0.8)

**Общие ключевые слова:** `docs`, `memory`, `ngt`, `ассоциативный`

**A:** `docs/05-habr-projects/01-synthesis.md`
> md) 29% - NGT^ngt Memory: ассоциативный граф(docs/05-habr-projects/memory/ngt-memory

**B:** `docs/05-habr-projects/02-collaboration-partners.md`
> md) 21% - NGT^ngt Memory: ассоциативный граф(docs/05-habr-projects/memory/ngt-memory

---

### 23. 🔢 Числовое — 21.0 vs 5.0 (уверенность: 0.8)

**Общие ключевые слова:** `docs`, `memory`, `ngt`, `ассоциативный`

**A:** `docs/05-habr-projects/02-collaboration-partners.md`
> md) 21% - NGT^ngt Memory: ассоциативный граф(docs/05-habr-projects/memory/ngt-memory

**B:** `docs/05-habr-projects/knowledge/wikontic.md`
> md) 21% - NGT^ngt Memory: ассоциативный граф(docs/05-habr-projects/memory/ngt-memory

---

### 24. 🔢 Числовое — 29.0 vs 17.0 (уверенность: 0.8)

**Общие ключевые слова:** `wikontic`, `граф`, `семантический`

**A:** `docs/05-habr-projects/memory/ngt-memory.md`
> md) 29% - Wikontic: семантический граф(

**B:** `docs/05-habr-projects/memory/yodoca.md`
> md) 17% - Wikontic: семантический граф(

---

### 25. 🔢 Числовое — 25.0 vs 739.0 (уверенность: 0.8)

**Общие ключевые слова:** `методика`, `отбора`, `рамка`

**A:** `docs/04-ai-collaborations/07-выводы.md`
> md) 25% - Методика и рамка отбора(02-методика-и-рамка-отбора

**B:** `docs/SIMILAR.md`
> 739 02-методика-и-рамка-отбора

---

### 26. 🔢 Числовое — 29.0 vs 739.0 (уверенность: 0.8)

**Общие ключевые слова:** `методика`, `отбора`, `рамка`

**A:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 29% - Методика и рамка отбора(02-методика-и-рамка-отбора

**B:** `docs/SIMILAR.md`
> 739 02-методика-и-рамка-отбора

---

### 27. 🔢 Числовое — 25.0 vs 17.0 (уверенность: 0.8)

**Общие ключевые слова:** `коллабораций`, `поиск`

**A:** `docs/01-svyazi/02-methodology.md`
> md) 25% - Поиск AI-коллабораций(

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 17% - Поиск AI-коллабораций(

---

### 28. 🔢 Числовое — 17.0 vs 25.0 (уверенность: 0.8)

**Общие ключевые слова:** `базе`, `знаний`, `противоречия`

**A:** `docs/01-svyazi/02-methodology.md`
> md) 17% - Противоречия в базе знаний(

**B:** `docs/04-ai-collaborations/02-методика-и-рамка-отбора.md`
> md) 25% - Противоречия в базе знаний(

---

### 29. 🔢 Числовое — 17.0 vs 29.0 (уверенность: 0.8)

**Общие ключевые слова:** `базе`, `знаний`, `противоречия`

**A:** `docs/01-svyazi/02-methodology.md`
> md) 17% - Противоречия в базе знаний(

**B:** `docs/04-ai-collaborations/08-что-это-продолжение-добавляет.md`
> md) 29% - Противоречия в базе знаний(

---

### 30. 🔢 Числовое — 80.0 vs 8.0 (уверенность: 0.8)

**Общие ключевые слова:** `coder`, `context`, `embed`, `nomic`

**A:** `docs/04-ai-collaborations/00-intro.md`
> qwen3-coder:30b, nomic-embed-text, 80k context, 8k max tokens, температура 0

**B:** `docs/05-habr-projects/memory/memnet.md`
> qwen3-coder:30b, nomic-embed-text, 80k context, 8k max tokens, температура 0

---

