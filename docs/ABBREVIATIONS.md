# Словарь аббревиатур и сокращений

<!-- abstract-auto -->
> **Абстракт** (авто)
>
> 🎯 **Проблема:** Найдено: 83 аббревиатур (22 известных + 61 автоматически) Аббревиатура Расшифровка Упоминаний -------------------------------------- ACD Automated Capability Discovery — ещё один с
> 🔧 **Подход:** Словарь аббревиатур и сокращений !TIP Этот документ описывает MVP-подход.
> ✅ **Результат:** Найдено: 83 аббревиатур (22 известных + 61 автоматически) Аббревиатура Расшифровка Упоминаний -------------------------------------- ACD Automated Capability Discovery — ещё один с
> 🏷️ **Ключевые слова:** `protocol`, `модель`, `source`, `language`, `model`, `конкретная`, `архитектура`, `неуточнено`
>


> [!TIP]
> Этот документ описывает MVP-подход. Начните с него для быстрого прототипа.

<!-- alert-added -->

**Найдено:** 83 аббревиатур (22 известных + 61 автоматически)

| Аббревиатура | Расшифровка | Упоминаний |
|-------------|-------------|------------|
| **ACD** | Automated Capability Discovery — ещё один сильный кубик: модель в роли «учёного» систематически генерирует задачи для мо | 4 |
| **ADR** | "ADR-004: Temporal Metadata as First-Class Concept" | 43 |
| **AGENTS** | типология + готовая к развёртыванию категория Type 1 | 8 |
| **AI** | это инфраструктурный слой для AI-managed virtual companies | 1946 |
| **AIRI** | серьёзная research лаборатория (Артём Шелманов и команда) | 16 |
| **ANP** | Agent Network Protocol | 4 |
| **API** ⭐ | Application Programming Interface — интерфейс программирования приложений | 218 |
| **BSL** ⭐ | Business Source License — бизнес-лицензия с открытым кодом | 22 |
| **CAMEL** | это другая значимая open-source framework, и сравнение их с Hermes будет показательным | 172 |
| **CI/CD** ⭐ | Continuous Integration / Continuous Deployment | 7 |
| **CLI** ⭐ | Command Line Interface — интерфейс командной строки | 43 |
| **CRDT** ⭐ | Conflict-free Replicated Data Type — структура данных без конфликтов слияния | 45 |
| **DAO** | результат смешанный | 2 |
| **EMEA** | RU/DE/EN на рабочем уровне, базирование в Германии, понимание европейского регуляторного контекста (что прямо читается ч | 25 |
| **ERROR** | MCP SDK not installed | 2 |
| **FAQ** ⭐ | Frequently Asked Questions — часто задаваемые вопросы | 0 |
| **FDE** | это исполнительская роль на чужую продуктовую повестку | 10 |
| **GDPR** ⭐ | General Data Protection Regulation — европейский регламент защиты данных | 44 |
| **GG** | они публичные) | 1 |
| **GUI** | -3 months effort | 13 |
| **HEAD** | 7 commits) | 1 |
| **HMP** | на когнитивной устойчивости и этике | 81 |
| **ID** | sgb:XII:90:4 (SGB XII, § 90, Abs | 6 |
| **II** | The Double-Triangle Architecture — formal описание дуальной структуры с вашей метафорой звезды Давида | 21 |
| **III** | Protocols Between Layers — три протокола с examples | 15 |
| **INPUT** | - Bescheid text (decoded by agent) | 1 |
| **IP** | AI-платформа, заказчик, сами фрилансеры? Сегодняшние legal frameworks не умеют отвечать на этот вопрос дёшево | 6 |
| **IV** | Nautilus Portal as Reference Implementation — как existing work serves как substrate | 18 |
| **IX** | 102 , sgg:86b:2 ), на прецеденты | 41 |
| **JWT** ⭐ | JSON Web Token — токен аутентификации | 2 |
| **KPI** | сколько полезных коллабораций, проектов, выступлений, mentorship‑пар или hiring‑контактов возникло из рекомендаций систе | 5 |
| **KSV** | потому что у них нет точных русских эквивалентов в контексте немецкой социально-правовой системы | 27 |
| **LAYER** | функциональная категория Type 4 | 41 |
| **LCI** | Lyapunov Coherence Index, target π | 21 |
| **LLM** ⭐ | Large Language Model — большая языковая модель | 289 |
| **LOC** | продублирована с разными строками в разных частях | 47 |
| **MCP** ⭐ | Model Context Protocol — протокол контекста для AI-инструментов | 657 |
| **MIT** ⭐ | Massachusetts Institute of Technology License — разрешительная лицензия | 156 |
| **ML** | несколько моделей → voting/averaging | 47 |
| **MMORPG** | это общее пространство , в котором вы видите аватары коллег, можете подойти к ним, стоять рядом, работать вместе в одной | 40 |
| **MRR** | это уже leverage для любого следующего шага, включая разговор с Anthropic Institute о grant/partnership | 2 |
| **MUST** | - Возвращать пустой список, если ничего не найдено (не `None`, не exception) | 77 |
| **MVP** ⭐ | Minimum Viable Product — минимально жизнеспособный продукт | 112 |
| **NDA** | intermediate view (placeholder'ы, но с consistent identifiers для longitudinal анализа) | 1 |
| **NLP** ⭐ | Natural Language Processing — обработка естественного языка | 0 |
| **NPP** | **федеративная модель**, где каждый | 86 |
| **OASIS** | до 1M agents simulation) | 3 |
| **ODT** | не только текст | 1 |
| **OKWF** | конкретная архитектура](#применение-к-okwf-конкретная-архитектура) | 262 |
| **OPTIONAL** | ключевые слова | 9 |
| **OS** | неуточнено | 116 |
| **OSS** ⭐ | Open Source Software — программное обеспечение с открытым кодом | 56 |
| **OUTPUT** | - Draft Widerspruch (DOCX format) | 1 |
| **P2P** ⭐ | Peer-to-Peer — децентрализованная сеть | 7 |
| **PARC** | research center, became iconic несмотря на parent brand decline OpenAI — research org становящаяся product company, name | 2 |
| **PII** ⭐ | Personally Identifiable Information — персональные данные | 28 |
| **PROTOCOL** | иначе future разработчики будут gадать | 208 |
| **PURE** | LLM-based User Profile Management for Recommender System» | 5 |
| **QA** | демон-критик (adversarial, rigorous) | 105 |
| **RAG** ⭐ | Retrieval-Augmented Generation — генерация с поиском по базе знаний | 329 |
| **README** | 550+ строк production-качества: установка, конфигурация для всех 6 платформ (включая детализацию /etc/fstab для CIFS, AD | 269 |
| **REQUIRED** | откуда пришло | 22 |
| **ROI** | 10 sec queries vs 2 hour manual search | 14 |
| **SDK** ⭐ | Software Development Kit — набор инструментов разработчика | 40 |
| **SENTINEL** | неуточнено; LiteLLM — MIT вне enterprise‑директорий; Auto AI Router — Apache 2 | 123 |
| **SF** | DC, Canberra) | 17 |
| **SGB** ⭐ | Sozialgesetzbuch — Социальный кодекс Германии | 317 |
| **SHOULD** | - Поддерживать case-insensitive matching для текстовых запросов | 33 |
| **SWE** | в Sales/Finance/Marketing/Legal зарплаты ниже и сильно варьируются по локации | 5 |
| **TF-IDF** ⭐ | Term Frequency–Inverse Document Frequency — метрика важности термина | 10 |
| **TODO** ⭐ | To Do — задача к выполнению | 3 |
| **TSU** | физика, MoME — математика; ZINC — software, гибридная архитектура — алгоритм; RISC-V — кремний, privacy — право; TinyML  | 9 |
| **TVCP** | Terminal Video Communication Platform, терминал видео коммуникация платформа, игры») — это необычное : Go + template + M | 1 |
| **UI** | -2 months effort | 60 |
| **URL** | я разберусь с любым вариантом именования | 59 |
| **VERIFY** | 6782 vs 6600] как метку | 1 |
| **VI** | Deployment Paths — humanities, project management, OSS, general | 3 |
| **VII** | Open Questions — governance, consent, economics, scale | 3 |
| **VIII** | Call to Action — что делать researchers, practitioners, founders | 2 |
| **VPS** | cron каждое утро обходит сайты Sozialgericht/BSG/KSV, генерирует Stellungnahme-черновики, обновляет статусы Aktenzeichen | 9 |
| **XII** | legally binding reference с нормативной силой | 34 |
| **YAML** ⭐ | YAML Ain't Markup Language — формат конфигурационных файлов | 102 |
| **ZINC** | - Ночью агент крутит эксперименты с промптами | 25 |

## Самые часто используемые

| Аббревиатура | Упоминаний |
|-------------|------------|
| **AI** | 1946 — _это инфраструктурный слой для AI-managed virtual companies_ |
| **MCP** | 657 — _Model Context Protocol — протокол контекста для AI-инструмен_ |
| **RAG** | 329 — _Retrieval-Augmented Generation — генерация с поиском по базе_ |
| **SGB** | 317 — _Sozialgesetzbuch — Социальный кодекс Германии_ |
| **LLM** | 289 — _Large Language Model — большая языковая модель_ |
| **README** | 269 — _550+ строк production-качества: установка, конфигурация для _ |
| **OKWF** | 262 — _конкретная архитектура](#применение-к-okwf-конкретная-архите_ |
| **API** | 218 — _Application Programming Interface — интерфейс программирован_ |
| **PROTOCOL** | 208 — _иначе future разработчики будут gадать_ |
| **CAMEL** | 172 — _это другая значимая open-source framework, и сравнение их с _ |
| **MIT** | 156 — _Massachusetts Institute of Technology License — разрешительн_ |
| **SENTINEL** | 123 — _неуточнено; LiteLLM — MIT вне enterprise‑директорий; Auto AI_ |
| **OS** | 116 — _неуточнено_ |
| **MVP** | 112 — _Minimum Viable Product — минимально жизнеспособный продукт_ |
| **QA** | 105 — _демон-критик (adversarial, rigorous)_ |

<!-- see-also -->

---

**Смотрите также:**
- [133-обратная-связь](docs/02-anthropic-vacancies/133-обратная-связь.md)
- [SEARCH](docs/SEARCH.md)

