...
## Тема 1: cowork, ingit, turn (2056 документов)
...
- ... и ещё 2041 документов

## Тема 4: triangle, double, domain (108 документов)

**Ключевые слова:** `triangle`, `double`, `domains`, `representative`, `layer`, `architecture`, `abstract`, `agent`, `domain`, `представительских`, `deployment`, `library`
...
- `docs/02-anthropic-vacancies/143-6-four-deployment-domains.md` — triangle, double, library, domain
...
## Тема 2: memory, wikontic, yodoca (104 документов)

**Ключевые слова:** `memory`, `yodoca`, `memnet`, `svyazi`, `wikontic`, `score`, `памяти`, `synthesis`, `граф`, `turn`, `guide`, `candidates`
...
- `docs/05-habr-projects/QA.md` — упоминаются, cardindex, episod, памяти
...
- `docs/PRIORITIES.md` — score, файл, memory, svyazi
...
## Тема 3: level, compatibility, bridges (82 документов)
...
- `docs/02-anthropic-vacancies/18-6-adapter-interface.md` — required, level, query, flow
- `docs/02-anthropic-vacancies/40-bridges.md` — bridges, compatibility, level, мост
- `docs/02-anthropic-vacancies/41-compatibility-level.md` — compatibility, level, passport, planned
...
## Тема 6: federation, normative, merging (68 документов)

**Ключевые слова:** `merging`, `normative`, `federation`, `contract`, `over`, `five`, `paths`, `onboarding`, `class`, `informative`, `adapter`, `portals`
...
- `docs/02-anthropic-vacancies/212-1-the-five-type-typology-of-principal-side-agents.md` — type, types, makes, table
...
- `docs/02-anthropic-vacancies/68-about.md` — portal, reference, protocol, repo
- `docs/02-anthropic-vacancies/81-6-adapter-interface.md` — required, baseadapter, safety, describe
- `docs/02-anthropic-vacancies/83-8-q6-space-normative.md` — class, mapping, вершины, neighbors
...
## Тема 5: section, beneficial, success (54 документов)

**Ключевые слова:** `section`, `beneficial`, `metrics`, `deployments`, `success`, `role`, `timeliness`, `dimension`, `lorenzo`, `proposer`, `concept`, `написание`

**Документы:**
- `docs/BROKEN_LINKS.md` — найден, внешние, показатели, компоненты
- `docs/COVERAGE.md` — условные, обозначения, heatmap, отсутствует
- `docs/HEALTH.md` — балл, broken, links, validation
- `docs/KNOWLEDGE_MAP.md` — секциям, быстрые, сущностей, корпус
...
- `docs/anthropic-vacancies/beneficial-deployments-concept/02-section-2-beneficial-dimension.md` — section, success, metrics, beneficial
...
| triangle, double, do | triangle | double | domains | representative | layer |
| memory, wikontic, yo | memory | yodoca | memnet | svyazi | wikontic |
| level, compatibility | level | compatibility | bridges | format | native |
| federation, normativ | merging | normative | federation | contract | over |
