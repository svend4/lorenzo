---
title: "Индекс обратных ссылок"
tags:
  - general
date: 2026-05-11
---

# Индекс обратных ссылок

**Файлов с входящими ссылками:** 2716

## Топ-30 самых цитируемых документов

| Документ | Входящих ссылок | Ссылающиеся файлы |
|----------|----------------|-------------------|
| `READABILITY` | 633 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `QA.md` +629 |
| `READING_TIME` | 620 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +616 |
| `OUTLINE` | 610 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `08-conclusions.md` +606 |
| `SEARCH` | 606 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +602 |
| `TABLES` | 397 | `00-intro-part2.md`, `03-component-catalog.md`, `06-security-privacy.md`, `QA.md` +393 |
| `README` | 335 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `03-portal-protocol-md.md` +331 |
| `README` | 301 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `03-portal-protocol-md.md` +297 |
| `components-by-name` | 102 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +98 |
| `CONCEPTS` | 96 | `02-methodology.md`, `12-roadmap.md`, `14-limitations.md`, `00-intro.md` +92 |
| `03-component-catalog` | 78 | `04-ensembles-overview.md`, `07-mvp-planning.md`, `09-architectural-gaps.md`, `10-second-order-ensembles.md` +74 |
| `01-интегральный-анализ-профиля-sven` | 73 | `00-intro.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md`, `103-appendix-b-change-log.md` +69 |
| `memnet` | 61 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md` +57 |
| `306-with-anthropic-s-cowork-platfor` | 59 | `00-intro.md`, `137-table-of-contents.md`, `146-acknowledgments.md`, `147-references.md` +55 |
| `GLOSSARY` | 55 | `03-local-first.md`, `04-sozialrecht-domain.md`, `ACTION_ITEMS.md`, `ALERTS.md` +51 |
| `HEALTH` | 52 | `ACTION_ITEMS.md`, `ALERTS.md`, `BROKEN_LINKS.md`, `CHANGELOG_AUTO.md` +48 |
| `ENTITIES` | 51 | `354-существующий-landscape-collaborators-твоя-working-.md`, `ACTION_ITEMS.md`, `ALERTS.md`, `CHANGELOG_AUTO.md` +47 |
| `METRICS` | 51 | `ACTION_ITEMS.md`, `ALERTS.md`, `BROKEN_LINKS.md`, `CHANGELOG_AUTO.md` +47 |
| `concepts` | 48 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +44 |
| `01-executive-summary` | 45 | `00-intro-part2.md`, `02-methodology.md`, `04-ensembles-overview.md`, `08-conclusions.md` +41 |
| `28-appendix-a-minimal-working-examp` | 43 | `03-portal-protocol-md.md`, `04-abstract.md`, `09-4-passport-passport-md.md`, `103-appendix-b-change-log.md` +39 |
| `authors-by-name` | 41 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +37 |
| `105-review-methodology-md` | 38 | `03-portal-protocol-md.md`, `04-abstract.md`, `106-tl-dr.md`, `108-2-формальный-workflow.md` +34 |
| `151-open-knowledge-work-foundation-` | 38 | `12-content-overview.md`, `13-angle-perspective.md`, `134-the-double-triangle-architecture-md.md`, `135-a-formal-model-for-human-ai-collaboration-in-distr.md` +34 |
| `25-13-reference-implementation` | 37 | `03-portal-protocol-md.md`, `05-0-status-of-this-document.md`, `104-appendix-c-references.md`, `122-глоссарий.md` +33 |
| `03-portal-protocol-md` | 37 | `05-0-status-of-this-document.md`, `105-review-methodology-md.md`, `123-portal-mcp-py.md`, `125-readme-mcp-md-инструкция-по-установке.md` +33 |
| `DECISIONS` | 35 | `03-component-catalog.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `memnet.md`, `yodoca.md` +31 |
| `123-portal-mcp-py` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `105-review-methodology-md.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +31 |
| `42-author-contact` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `05-0-status-of-this-document.md`, `105-review-methodology-md.md` +31 |
| `ACTION_ITEMS` | 34 | `03-component-catalog.md`, `01-интегральный-анализ-профиля-svend4.md`, `115-8-ограничения-и-открытые-вопросы.md`, `133-обратная-связь.md` +30 |
| `04-abstract` | 34 | `05-0-status-of-this-document.md`, `09-4-passport-passport-md.md`, `105-review-methodology-md.md`, `123-portal-mcp-py.md` +30 |

## Ссылки по разделам

| Раздел | Входящих | Исходящих |
|--------|----------|-----------|
| **.claude** | 56 | 0 |
| **01-svyazi** | 386 | 177 |
| **02-anthropic-vacancies** | 7074 | 4920 |
| **03-technology-combinations** | 87 | 53 |
| **04-ai-collaborations** | 291 | 174 |
| **05-habr-projects** | 270 | 146 |
| **ai-collaborations** | 320 | 288 |
| **anthropic-vacancies** | 1253 | 1105 |
| **autofilled** | 218 | 177 |
| **badges** | 12 | 12 |
| **contacts** | 239 | 170 |
| **glossary** | 198 | 200 |
| **habr-unique-projects** | 629 | 507 |
| **letters** | 51 | 55 |
| **lorenzo-agent** | 639 | 642 |
| **meta-scripting** | 67 | 67 |
| **nautilus** | 2545 | 2470 |
| **obsidian** | 5126 | 170 |
| **processing-guide** | 159 | 113 |
| **root** | 4674 | 13141 |
| **scripts** | 2 | 0 |
| **svyazi-2-0** | 672 | 526 |
| **technology-combinations** | 641 | 483 |
| **templates** | 216 | 229 |

<!-- similar-docs -->

---

**Похожие документы:**
- [BACKLINKS](../BACKLINKS.md) (сходство 0.84)
- [DIGEST_WEEKLY](DIGEST_WEEKLY.md) (сходство 0.31)
- [COMPLEXITY](../COMPLEXITY.md) (сходство 0.30)

