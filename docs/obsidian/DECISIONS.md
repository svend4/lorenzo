...
- Svyazi + AgentFS + NGT/Yodoca + LiteParse: даёт уже полезный MVP 2. > 🏷️ **Ключевые слова:** `svyazi`, `проект`, `cardindex`, `agentfs`, `добавляет`, `продолжение`, `rufler`, `memory` > > [!IMPORTAN  
  _→ 01-executive-summary_
...
- слой — не память, не RAG[^rag] и не оркестр **Проекты:** Svyazi[^svyazi], CardIndex[^cardindex], AgentFS[^agentfs], mclaude, AI Factory, Rufler[^rufler], LiteParse, Yodoca[^  
  _→ 08-conclusions_

- слой — не память, не RAG и не оркестрация по отдельности: все они уже представлены на Хабре и в репозиториях. Дефицитный слой — **правильная сборка**: где CardIndex остаётся  
  _→ 08-conclusions_
...
- слой — не память, не RAG ^rag и не оркестр Проекты: Svyazi ^svyazi , CardIndex ^cardindex , AgentFS ^agentfs , mclaude, AI Factory, Rufler ^rufler , LiteParse 01-executive-summary.md , Yodoca ^yodoca  
  _→ SUMMARIES_
...
_...ещё 64 записей в этой категории_


## Mvp (16)

- читать перед погружением в детали. > Если идти дальше после базового MVP, то лучшая стратегия — не “добавить всё”, а пройти **три короткие итерации**, каждая из которых поднимает один новый класс сво  
  _→ 12-roadmap_
...
- критических** | **3** | ## PII и секреты ✅ PII и секреты не найдены. ## Небезопасный код | Файл | Строка | Проблема | Фрагмент | |------|--------|----------|----------| | `scripts/gateway.py` | 6  
  _→ SENTINEL_

- замена на HTTPS._ - `http://localhost:8000````` в `docs/SENTINEL.md` - `http://localhost:8000`````` в `docs/SENTINEL.md` - `http://localhost:8080````` в `docs/SENTINEL.md` - `http://localhost:8080```  
  _→ SENTINEL_

- критических** | **0** | ### 282. Лицензионные риски _Файл: `docs/obsidian/SENTINEL.md` | 3 колонок, 4 строк_ | Файл | Лицензия | Риск | |------|----------|------| | `docs/obsidian/02-anthropic-vaca  
  _→ TABLES_

- критических** | **0** | ### 180. Лицензионные риски _Файл: `docs/SENTINEL.md` | 3 колонок, 4 строк_ | Файл | Лицензия | Риск | |------|----------|------| | `docs/obsidian/02-anthropic-vacancies/365  
  _→ TABLES_
...
- склеивать]]](#что-пока-лучше-не-склеиватьsvyazi-2-0limitationsdo-not-gluemd) - [[do-not-glue|[Лицензионные развилки]] > - Оркестрация — выбрать один spine(#оркестрация-выбрать-один-spine) - Contents - Оркестрация  
  _→ OUTLINE_
...
- склеивать в один релиз" tags: - ai-collaborations date: 2026-05-11 --- # Что пока не стоит склеивать в один релиз <!-- toc-auto --> ## Contents -  
  _→ 09-do-not-glue_
...
_...ещё 1009 записей в этой категории_
