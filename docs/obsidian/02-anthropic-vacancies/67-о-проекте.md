---
title: "🇷🇺 О проекте"
tags:
  - rag
  - architecture
  - collaboration
  - anthropic-vacancies
date: 2026-05-14
---

# 🇷🇺 О проекте

> [!NOTE]
> Документ создан на основе исследования. Ссылки ведут на связанные материалы.

<!-- alert-added -->

<!-- abstract-auto -->
> **Абстракт** (авто)
>
> 🎯 **Проблема:** Документация - PORTAL-PROTOCOL.md(03-portal-protocol-md.md) — formal specification v1.0 (15 страниц) - passports/(./passports/) — описания репо экосистемы для людей - info1(./passports
> 🔧 **Подход:** - 0: Discoverable — только объявлен в registry - 1: Readable — имеет passport + describe() метод - 2: Queryable — реализует fetch(query) — поиск работает - 3: Interactive — transla
> ✅ **Результат:** Это создаёт lock-in, теряет авторские модели, гомогенизирует разные типы знания.
> 🏷️ **Ключевые слова:** `nautilus`, `svend`, `passports`, `portal`, `terminology`, `protocol`, `anthropic`, `vacancies`
>


<!-- summary -->
> [!NOTE]
> Документ создан на основе исследования. Ссылки ведут на связанные материалы. 🎯 Проблема: Документация - PORTAL-PROTOCOL.md(03-portal-protocol-md.md) — formal specification v1.0 (15 страниц) - passports/(./passports/) — описания репо экосистемы для людей - info1(./passports
> Документ создан на основе исследования. Ссылки ведут на связанные материалы.

> Nautilus — протокол и reference implementation для **федерации**

---

<!-- toc -->
## Содержание

- [🇷🇺 О проекте](#о-проекте)
  - [Философия](#философия)
  - [Эталонная экосистема: svend4](#эталонная-экосистема-svend4)
  - [Быстрый старт](#быстрый-старт)
  - [Подключить свой репозиторий](#подключить-свой-репозиторий)
  - [Как это работает](#как-это-работает)
  - [Уровни совместимости](#уровни-совместимости)
  - [Документация](#документация)
  - [Файловая структура](#файловая-структура)
  - [Статус проекта](#статус-проекта)
  - [Лицензии](#лицензии)
  - [Контакт](#контакт)

---

<!-- tags: rag, architecture, collaboration -->




## 🇷🇺 О проекте

Nautilus — протокол и reference implementation для **федерации** 
независимых git-репозиториев, содержащих знания в разных форматах, 
без принудительного приведения к единой схеме.

Подобно тому, как Office Suite читает `.docx`, `.pdf`, `.xlsx` 
без слияния их в один формат — Nautilus читает репозитории 
экосистемы, находит связи между ними и строит общий поисковый слой.

### Философия

Большинство современных систем знаний (Notion, Obsidian, Roam, 
Coda) требуют миграции данных в их единый формат. Это создаёт 
lock-in, теряет авторские модели, гомогенизирует разные типы 
знания.

**Nautilus выбирает обратный путь:**

- Каждый репозиторий сохраняет свой native-формат
- Перевод между форматами — через адаптеры, по запросу
- Консенсус считается по пересечению результатов из разных репо
- Добавление нового репо занимает 5 минут и не требует рефакторинга

### Эталонная экосистема: svend4

Reference implementation демонстрируется на трёх репо:

| Репо | Формат | Содержание | Угол зрения |
| --- | --- | --- | --- |
| [info1](https://github.com/svend4/info1) | `.info1` | 74 документа с α-уровнями | Методологический |
| [pro2](https://github.com/svend4/pro2) | `.pro2` | Q6-граф концептов, 64 вершины | Семантический |
| [meta](https://github.com/svend4/meta) | `.meta` | 256 CA-правил + 64 гексаграммы | Символьный |

Триада углов не случайна — это классическая семиотическая 
структура Пирса (pragmatic × semantic × syntactic), реализованная 
как federated knowledge ecosystem.

### Быстрый старт

```bash
git clone https://github.com/svend4/nautilus
cd nautilus
pip install -r requirements.txt

# CLI
python portal.py --query "кристалл"

# Веб-интерфейс
python portal.py --serve
# открыть http://localhost:8000

# MCP для Claude Desktop (в разработке)
# см. MCP-EXTENSION.md
```

### Подключить свой репозиторий

**Минимально (Level 0–1, 5 минут):**

1. Положите `[[memnet|nautilus]].json` в корень вашего репо:

```json
{
  "name": "my-repo",
  "format": ".my-format",
  "native_unit": "что хранится",
  "bridges": {
    "info1": "как концепты соотносятся",
    "pro2": "...",
    "meta": "..."
  }
}
```

2. Напишите `passport.md` — одну страницу описания (см. шаблон в 
   `passports/`)

**Полностью (Level 2–3, несколько часов):**

3. Добавьте адаптер в `adapters/my_repo.py`, реализующий 
   `[[01-интегральный-анализ-профиля-svend4|BaseAdapter]]` интерфейс (см. [[03-portal-protocol-md|PORTAL-PROTOCOL.md §6]])

4. Зарегистрируйте репо в `[[memnet|nautilus]].json` корневого портала

### Как это работает

```
┌──────────┐  ┌──────────┐  ┌──────────┐
│  info1   │  │   pro2   │  │   meta   │
│ α-уровни │  │ Q6-граф  │  │CA-правила│
└────┬─────┘  └────┬─────┘  └────┬─────┘
     │             │             │
     └─────────────┼─────────────┘
                   │
           ⬡ Nautilus Portal
       (адаптеры + консенсус)
                   │
         ┌─────────┼─────────┐
         │         │         │
       CLI       Web       MCP
                        (для Claude)
```

**Консенсус** считается по пересечению: концепт «full consensus», 
если найден во всех опрошенных репо; «partial» — в части; 
«singular» — только в одном. Отчёт всегда показывает, откуда 
пришёл каждый результат.

### Уровни совместимости

Репо подключается постепенно — не обязательно сразу на максимум.

- **0: Discoverable** — только объявлен в registry
- **1: Readable** — имеет passport + `describe()` метод
- **2: Queryable** — реализует `fetch(query)` — поиск работает
- **3: Interactive** — `translate_to()` — cross-repo переводы через 
  bridges

Детали — в [[03-portal-protocol-md|PORTAL-PROTOCOL.md §5]].

### Документация

- **[[03-portal-protocol-md|PORTAL-PROTOCOL.md]]** — formal 
  specification v1.0 (15 страниц)
- **`passports/`** — описания репо экосистемы для 
  людей
  - `info1`
  - `pro2`
  - `meta`
- **Issues** — вопросы, предложения, сообщения об ошибках

### Файловая структура

```
nautilus/
├── README.md              ← этот файл (RU/EN)
├── PORTAL-PROTOCOL.md     ← formal specification v1.0
├── portal.py              ← главный движок
├── nautilus.json          ← registry репо экосистемы
├── requirements.txt       ← Python зависимости
├── index.html             ← GitHub Pages (статический портал)
├── glyph_adapter.py       ← мета-адаптер символьного уровня
├── adapters/              ← по одному адаптеру на репо
│   ├── base.py            ← BaseAdapter + PortalEntry
│   ├── info1.py
│   ├── pro2.py
│   └── meta.py
└── passports/             ← human-readable описания репо
    ├── info1.md
    ├── pro2.md
    └── meta.md
```

### Статус проекта

Nautilus находится в **активной разработке**:

- ✅ Протокол v1.0 (draft) — спецификация написана
- ✅ Reference implementation — портал работает с тремя репо
- 🔄 MCP-обёртка — в работе
- 🔄 PyPI packaging — планируется
- 🔄 Arxiv preprint — планируется
- ⏳ Submission в [MCP Registry](https://github.com/mcp) — после 
  завершения MCP-обёртки

**Текущая версия**: v1.0.0-draft. Breaking changes возможны до 
v1.0.0-stable.

### Лицензии

- **Документация** (README, PORTAL-PROTOCOL, passports): 
  [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
- **Код** (portal, adapters, glyph_adapter): 
  [[README|MIT]]

### Контакт

- **Автор**: svend4
- **Связаться**: через [Issues](https://github.com/svend4/nautilus/issues)

---

<!-- similar-docs -->

---

## Похожие документы
- [[68-about]] (сходство 0.17)
- [[07-2-terminology]] (сходство 0.16)
- [[77-2-terminology]] (сходство 0.14)


<!-- see-also -->

---

## Смотрите также
- [[68-about]]
- [[07-2-terminology]]
- [[77-2-terminology]]
- [[76-1-introduction]]

<!-- backlinks-auto -->
## Упоминается в

- [[06-1-introduction|1. Introduction]]
- [[76-1-introduction|1. Introduction]]
- [[07-2-terminology|2. Terminology]]
- [[77-2-terminology|2. Terminology]]
- [[97-22-glossary-of-reference-examples|22. Glossary of Reference Examples]]
- [[64-for-the-curious-philosophy|For the Curious: Philosophy]]
- [[README|Вакансии Anthropic — Анализ по кластерам]]
- [[02-общий-план-развития-nautilus-portal-protocol|ОБЩИЙ ПЛАН РАЗВИТИЯ NAUTILUS PORTAL PROTOCOL]]
- [[68-about|🇬🇧 About]]
## Упоминается в

- [[README|Вакансии Anthropic — Анализ по кластерам]]

<!-- related-auto -->
## Связанные документы

- [[68-about|🇬🇧 About]] _29%_
- [[25-13-reference-implementation|13. Reference Implementation]] _25%_
- [[42-author-contact|Author & Contact]] _21%_
- [[64-for-the-curious-philosophy|For the Curious: Philosophy]] _21%_
- [[02-общий-план-развития-nautilus-portal-protocol|ОБЩИЙ ПЛАН РАЗВИТИЯ NAUTILUS PORTAL PROTOCOL]] _17%_
- [[27-15-glossary-of-examples|15. Glossary of Examples]] _17%_
- [[35-passports-info1-md|passports/info1.md]] _17%_
- [[46-essence|Essence]] _17%_
## Связанные документы

- [[68-about|🇬🇧 About]] _33%_
- [[25-13-reference-implementation|13. Reference Implementation]] _29%_
- [[42-author-contact|Author & Contact]] _29%_
- [[93-18-reference-implementation|18. Reference Implementation]] _29%_
- [[104-appendix-c-references|Appendix C: References]] _25%_
- [[105-review-methodology-md|REVIEW_METHODOLOGY.md]] _25%_
- [[122-глоссарий|Глоссарий]] _25%_
- [[141-4-nautilus-portal-as-reference-substrate|4. Nautilus Portal as Reference Substrate]] _25%_

<!-- backlinks -->

---

## Кто ссылается на этот документ (10)
- 02-общий-план-развития-[[02-общий-план-развития-nautilus-portal-protocol|nautilus-portal-protocol]]
- [[104-appendix-c-references]]
- [[105-review-methodology-md]]
- [[122-глоссарий]]
- 141-4-[[141-4-nautilus-portal-as-reference-substrate|nautilus-portal-as-reference-substrate]]
- [[25-13-reference-implementation]]
- [[42-author-contact]]
- [[64-for-the-curious-philosophy]]
- _...ещё 2_

