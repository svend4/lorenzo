---
template: legal-case
version: "1.0"
case_id: "[Aktenzeichen]"
case_type: sozialgericht
status: open
filed: null
deadline: null
parties: []
tags: [legal, case]
---

# Юридический кейс: [Aktenzeichen]

<!-- summary: О чём кейс в 1-2 предложениях -->
<!-- tags: legal, case -->

## Идентификация

| Параметр | Значение |
|----------|---------|
| Aktenzeichen | [A 1 SO 123/24] |
| Тип | [Sozialgericht / Verwaltungsgericht / …] |
| Stage | [Antrag / Bescheid / Widerspruch / Klage / Berufung] |
| Filed | [дата] |
| Deadline | [дата] |
| Status | [open / closed / appeal] |

## Стороны

| Роль | Кто |
|------|-----|
| Antragsteller / Kläger | [имя] |
| Behörde / Beklagte | [имя] |

## Хронология

| Дата | Событие | Документ |
|------|---------|----------|
| YYYY-MM-DD | Antrag eingereicht | [файл] |
| YYYY-MM-DD | Bescheid erhalten | [файл] |
| YYYY-MM-DD | Widerspruch | [файл] |

## Предмет спора

[Краткое описание сути 3-5 предложений.]

## Применимые нормы (§§)

- § XXX SGB Y — [пояснение]
- § ZZZ AusländerG — [пояснение]

## Аргументы

### Antragsteller / Kläger

1. [Аргумент 1]
2. [Аргумент 2]

### Behörde / Beklagte

1. [Контр-аргумент 1]
2. [Контр-аргумент 2]

## Прецеденты

| Решение | Суд | Дата | Релевантность |
|---------|-----|------|---------------|
| [BSG B 14 AS 1/22 R] | BSG | 2022-XX | [как относится] |

## Текущий статус

[Что происходит сейчас. Кто следующий должен ходить.]

## Открытые вопросы

- ?

## Действия

- [ ] [Action 1] — отв. [имя], срок [дата]
- [ ] [Action 2]

## Связанные кейсы

- [`docs/legal/cases/<other>.md`](...)

---
_Создано: 2026-04-29_

<!-- see-also -->

---

**Смотрите также:**
- [`docs/svyazi-2-0/components/legal-rag.md`](...)
- [decision-record](docs/templates/decision-record.md)
