# Индекс обратных ссылок

<!-- summary -->
> **Файлов с входящими ссылками:** 2722
**Проекты:** Svyazi, MemNet

---
<!-- tags: memory, ingestion, local-first, roadmap, anthropic, collaboration -->




**Файлов с входящими ссылками:** 2722

## Топ-30 самых цитируемых документов

| Документ | Входящих ссылок | Ссылающиеся файлы |
|----------|----------------|-------------------|
| `READABILITY` | 1365 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `QA.md` +1361 |
| `READING_TIME` | 1341 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +1337 |
| `SEARCH` | 1324 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +1320 |
| `TABLES` | 1111 | `00-intro-part2.md`, `03-component-catalog.md`, `06-security-privacy.md`, `QA.md` +1107 |
| `OUTLINE` | 694 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `08-conclusions.md` +690 |
| `README` | 336 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `03-portal-protocol-md.md` +332 |
| `README` | 299 | `QA.md`, `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md` +295 |
| `CONCEPTS` | 110 | `02-methodology.md`, `12-roadmap.md`, `14-limitations.md`, `00-intro.md` +106 |
| `components-by-name` | 105 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +101 |
| `03-component-catalog` | 78 | `04-ensembles-overview.md`, `07-mvp-planning.md`, `09-architectural-gaps.md`, `10-second-order-ensembles.md` +74 |
| `01-интегральный-анализ-профиля-sven` | 74 | `00-intro.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md`, `103-appendix-b-change-log.md` +70 |
| `GLOSSARY` | 63 | `QA.md`, `QA.md`, `03-local-first.md`, `04-sozialrecht-domain.md` +59 |
| `HEALTH` | 62 | `QA.md`, `QA.md`, `QA.md`, `QA.md` +58 |
| `ENTITIES` | 61 | `QA.md`, `354-существующий-landscape-collaborators-твоя-working-.md`, `QA.md`, `QA.md` +57 |
| `memnet` | 61 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md` +57 |
| `METRICS` | 59 | `QA.md`, `QA.md`, `QA.md`, `QA.md` +55 |
| `306-with-anthropic-s-cowork-platfor` | 59 | `00-intro.md`, `137-table-of-contents.md`, `146-acknowledgments.md`, `147-references.md` +55 |
| `concepts` | 50 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +46 |
| `01-executive-summary` | 45 | `00-intro-part2.md`, `02-methodology.md`, `04-ensembles-overview.md`, `08-conclusions.md` +41 |
| `authors-by-name` | 44 | `AUTHORS.md`, `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md` +40 |
| `28-appendix-a-minimal-working-examp` | 42 | `03-portal-protocol-md.md`, `04-abstract.md`, `09-4-passport-passport-md.md`, `103-appendix-b-change-log.md` +38 |
| `105-review-methodology-md` | 38 | `03-portal-protocol-md.md`, `04-abstract.md`, `106-tl-dr.md`, `108-2-формальный-workflow.md` +34 |
| `151-open-knowledge-work-foundation-` | 38 | `12-content-overview.md`, `13-angle-perspective.md`, `134-the-double-triangle-architecture-md.md`, `135-a-formal-model-for-human-ai-collaboration-in-distr.md` +34 |
| `25-13-reference-implementation` | 37 | `03-portal-protocol-md.md`, `05-0-status-of-this-document.md`, `104-appendix-c-references.md`, `122-глоссарий.md` +33 |
| `03-portal-protocol-md` | 37 | `05-0-status-of-this-document.md`, `105-review-methodology-md.md`, `123-portal-mcp-py.md`, `125-readme-mcp-md-инструкция-по-установке.md` +33 |
| `ACTION_ITEMS` | 35 | `03-component-catalog.md`, `01-интегральный-анализ-профиля-svend4.md`, `115-8-ограничения-и-открытые-вопросы.md`, `133-обратная-связь.md` +31 |
| `123-portal-mcp-py` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `105-review-methodology-md.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +31 |
| `42-author-contact` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `05-0-status-of-this-document.md`, `105-review-methodology-md.md` +31 |
| `04-abstract` | 35 | `05-0-status-of-this-document.md`, `09-4-passport-passport-md.md`, `105-review-methodology-md.md`, `123-portal-mcp-py.md` +31 |
| `reading-paths` | 35 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `README.md` +31 |

## Ссылки по разделам

| Раздел | Входящих | Исходящих |
|--------|----------|-----------|
| **.claude** | 28 | 0 |
| **01-svyazi** | 384 | 183 |
| **02-anthropic-vacancies** | 7122 | 4972 |
| **03-technology-combinations** | 100 | 61 |
| **04-ai-collaborations** | 287 | 178 |
| **05-habr-projects** | 276 | 162 |
| **ROADMAP** | 49 | 42 |
| **ai-collaborations** | 375 | 374 |
| **anthropic-vacancies** | 1452 | 1396 |
| **autofilled** | 235 | 206 |
| **badges** | 12 | 7 |
| **contacts** | 242 | 184 |
| **glossary** | 211 | 214 |
| **habr-unique-projects** | 733 | 657 |
| **letters** | 105 | 113 |
| **lorenzo-agent** | 738 | 806 |
| **meta-scripting** | 72 | 90 |
| **nautilus** | 2913 | 3155 |
| **obsidian** | 6491 | 3645 |
| **processing-guide** | 160 | 149 |
| **root** | 7802 | 13337 |
| **scripts** | 2 | 0 |
| **svyazi-2-0** | 771 | 699 |
| **technology-combinations** | 749 | 609 |
| **templates** | 193 | 263 |
