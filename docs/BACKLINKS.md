# Индекс обратных ссылок

> [!WARNING]
> Документ содержит описание рисков и ограничений. Изучите их перед принятием архитектурных решений.

<!-- alert-added -->

**Файлов с входящими ссылками:** 2715

## Топ-30 самых цитируемых документов

| Документ | Входящих ссылок | Ссылающиеся файлы |
|----------|----------------|-------------------|
| `READABILITY` | 684 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `QA.md` +680 |
| `READING_TIME` | 668 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +664 |
| `OUTLINE` | 667 | `00-intro-part2.md`, `02-methodology.md`, `06-security-privacy.md`, `08-conclusions.md` +663 |
| `SEARCH` | 659 | `02-methodology.md`, `06-security-privacy.md`, `QA.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +655 |
| `TABLES` | 452 | `00-intro-part2.md`, `03-component-catalog.md`, `06-security-privacy.md`, `QA.md` +448 |
| `README` | 336 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `03-portal-protocol-md.md` +332 |
| `README` | 291 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `03-portal-protocol-md.md` +287 |
| `components-by-name` | 103 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +99 |
| `CONCEPTS` | 94 | `02-methodology.md`, `12-roadmap.md`, `14-limitations.md`, `00-intro.md` +90 |
| `03-component-catalog` | 76 | `04-ensembles-overview.md`, `07-mvp-planning.md`, `09-architectural-gaps.md`, `10-second-order-ensembles.md` +72 |
| `01-интегральный-анализ-профиля-sven` | 73 | `00-intro.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md`, `103-appendix-b-change-log.md` +69 |
| `memnet` | 61 | `00-intro.md`, `01-интегральный-анализ-профиля-svend4.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `07-2-terminology.md` +57 |
| `306-with-anthropic-s-cowork-platfor` | 58 | `00-intro.md`, `137-table-of-contents.md`, `146-acknowledgments.md`, `147-references.md` +54 |
| `GLOSSARY` | 53 | `03-local-first.md`, `04-sozialrecht-domain.md`, `ACTION_ITEMS.md`, `CHANGELOG_AUTO.md` +49 |
| `HEALTH` | 50 | `ACTION_ITEMS.md`, `BROKEN_LINKS.md`, `CHANGELOG_AUTO.md`, `CITATION_INDEX.md` +46 |
| `ENTITIES` | 49 | `354-существующий-landscape-collaborators-твоя-working-.md`, `ACTION_ITEMS.md`, `CHANGELOG_AUTO.md`, `CITATION_INDEX.md` +45 |
| `METRICS` | 48 | `ACTION_ITEMS.md`, `BROKEN_LINKS.md`, `CHANGELOG_AUTO.md`, `CITATION_INDEX.md` +44 |
| `concepts` | 48 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +44 |
| `01-executive-summary` | 45 | `00-intro-part2.md`, `02-methodology.md`, `04-ensembles-overview.md`, `08-conclusions.md` +41 |
| `28-appendix-a-minimal-working-examp` | 42 | `03-portal-protocol-md.md`, `04-abstract.md`, `09-4-passport-passport-md.md`, `103-appendix-b-change-log.md` +38 |
| `authors-by-name` | 41 | `OUTLINE.md`, `READABILITY.md`, `READING_TIME.md`, `SEARCH.md` +37 |
| `DECISIONS` | 38 | `03-component-catalog.md`, `02-общий-план-развития-nautilus-portal-protocol.md`, `memnet.md`, `yodoca.md` +34 |
| `105-review-methodology-md` | 38 | `03-portal-protocol-md.md`, `04-abstract.md`, `106-tl-dr.md`, `108-2-формальный-workflow.md` +34 |
| `151-open-knowledge-work-foundation-` | 38 | `12-content-overview.md`, `13-angle-perspective.md`, `134-the-double-triangle-architecture-md.md`, `135-a-formal-model-for-human-ai-collaboration-in-distr.md` +34 |
| `25-13-reference-implementation` | 37 | `03-portal-protocol-md.md`, `05-0-status-of-this-document.md`, `104-appendix-c-references.md`, `122-глоссарий.md` +33 |
| `03-portal-protocol-md` | 37 | `05-0-status-of-this-document.md`, `105-review-methodology-md.md`, `123-portal-mcp-py.md`, `125-readme-mcp-md-инструкция-по-установке.md` +33 |
| `ACTION_ITEMS` | 35 | `03-component-catalog.md`, `01-интегральный-анализ-профиля-svend4.md`, `115-8-ограничения-и-открытые-вопросы.md`, `133-обратная-связь.md` +31 |
| `123-portal-mcp-py` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `105-review-methodology-md.md`, `110-вопрос-fallback-ratio-как-критический-или-осмыслен.md` +31 |
| `42-author-contact` | 35 | `03-portal-protocol-md.md`, `04-abstract.md`, `05-0-status-of-this-document.md`, `105-review-methodology-md.md` +31 |
| `CONTACTS` | 34 | `13-contacts.md`, `13-контактная-стратегия-и-узкие-вопросы-для-авторов.md`, `ACTION_ITEMS.md`, `CROSS_SECTION.md` +30 |

## Ссылки по разделам

| Раздел | Входящих | Исходящих |
|--------|----------|-----------|
| **.claude** | 56 | 0 |
| **01-svyazi** | 373 | 182 |
| **02-anthropic-vacancies** | 7082 | 4971 |
| **03-technology-combinations** | 89 | 59 |
| **04-ai-collaborations** | 275 | 177 |
| **05-habr-projects** | 273 | 160 |
| **ai-collaborations** | 323 | 374 |
| **anthropic-vacancies** | 1272 | 1394 |
| **autofilled** | 229 | 206 |
| **badges** | 14 | 15 |
| **contacts** | 240 | 183 |
| **glossary** | 203 | 214 |
| **habr-unique-projects** | 639 | 657 |
| **letters** | 107 | 114 |
| **lorenzo-agent** | 650 | 804 |
| **meta-scripting** | 71 | 88 |
| **nautilus** | 2623 | 3155 |
| **obsidian** | 6506 | 170 |
| **processing-guide** | 163 | 147 |
| **root** | 4952 | 13094 |
| **scripts** | 2 | 0 |
| **svyazi-2-0** | 697 | 699 |
| **technology-combinations** | 662 | 609 |
| **templates** | 232 | 261 |

<!-- similar-docs -->

---

**Похожие документы:**
- [BACKLINKS](obsidian/BACKLINKS.md) (сходство 0.76)
- [SEE_ALSO](SEE_ALSO.md) (сходство 0.26)
- [SEE_ALSO](obsidian/SEE_ALSO.md) (сходство 0.25)

