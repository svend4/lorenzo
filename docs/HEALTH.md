# Health Dashboard

<!-- summary -->
> Балл здоровья репозитория: **99/100** — файлов: 2785, слов: 3,042,303

<!-- tags: health, quality, metrics, documentation -->

> [!TIP]
> Балл 90+ означает отличное состояние базы знаний.

<!-- alert-added -->

_Обновлено: 2026-05-29_

## Общий балл: **99/100** 🟢

## Метрики

| Метрика | Значение | Статус | Балл |
|---------|----------|--------|------|
| Покрытие текста | 97.6% | 🟢 | 98 |
| Полнота тем | 25✅ 0⚠️ 0❌ | 🟢 | 100 |
| Согласованность | 0 проблем | 🟢 | 100 |
| Внутренние ссылки | 18 сломано | 🟢 | 99 |
| Дублирование | 0 точных дублей | 🟢 | 100 |

## Структура репозитория

**Файлов:** 2785  
**Слов:** 3,042,303

| Раздел | Файлов |
|--------|--------|
| 01-svyazi | 16 |
| 02-anthropic-vacancies | 357 |
| 03-technology-combinations | 7 |
| 04-ai-collaborations | 40 |
| 05-habr-projects | 16 |
| 06-discovery | 260 |
| ROADMAP | 7 |
| ai-collaborations | 31 |
| anthropic-vacancies | 111 |
| autofilled | 13 |
| badges | 1 |
| contacts | 32 |
| enriched | 49 |
| glossary | 4 |
| habr-unique-projects | 56 |
| letters | 10 |
| lorenzo-agent | 62 |
| meta-scripting | 7 |
| nautilus | 255 |
| obsidian | 1166 |
| processing-guide | 13 |
| rfcs | 5 |
| root | 130 |
| svyazi-2-0 | 60 |
| technology-combinations | 53 |
| templates | 24 |

## Action Items

Извлечено действий: **3475**  
Пробелов знаний: **0**  
Сломанных ссылок: **18**

## Скрипты обработки

Скриптов в `scripts/`: **221**

## Рекомендации

- 🔗 Проверить 18 сломанных ссылок (`BROKEN_LINKS.md`)
- ✅ Репозиторий в отличном состоянии!

## Смотрите также

- [METRICS](METRICS.md) — метрики качества документов
- [BROKEN_LINKS](BROKEN_LINKS.md) — состояние внутренних ссылок
- [VALIDATION](VALIDATION.md) — валидация структуры
- [SCORING](SCORING.md) — готовность к запуску (Go/No-Go)
