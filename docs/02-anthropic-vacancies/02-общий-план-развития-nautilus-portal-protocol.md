...
<!-- summary -->
> * Это low barrier to entry : подключить существующий репо к Nautilus занимает 5 минут, не требует рефакторинга. * Не привязан к конкретным именам классов из существующего код — фиксирую архитектурные контракты , а не конкретный код
 ...
 Знает свои bridges — как его концепты мапятся на конц
...
- [ОБЩИЙ ПЛАН РАЗВИТИЯ NAUTILUS PORTAL PROTOCOL](#общий-план-развития-nautilus-portal-protocol)
...
- README пор pro2/nautilus/ (embedded-версия, 112 строк) — архитектурная спецификация v1.0 с философией federation-over-merging, триадой [info1](01-интегральный-анализ-профиля-svend4.md)/pro2/meta, четырёхуровневой compatibility scheme, consensus-механизмом
...
- [pro2](01-интегральный-анализ-профиля-svend4.md) — Q6-концепты, граф знаний, семантический угол, YiJing-Transformer v53–v61, LCI target π
- meta — 256 CA-правил, гексаграммы, символьный угол
- 70 репозиториев в экосистеме svend4 , созданных за 4 месяца
- Claude Code как сооруденно-разработчик репо [nautilus](../05-habr-projects/memory/memnet.md)
...
- Что такое [nautilus](../05-habr-projects/memory/memnet.md).json — реестр репо
- Что такое passport.md — описание репо для людей и для адаптеров
- Что такое адаптер и какой у него обязательный интерфейс
- Что такое [PortalEntry](01-интегральный-анализ-профиля-svend4.md) — unified data structure, в которую адаптеры конвертируют native-форматы
...
Это [info1](01-интегральный-анализ-профиля-svend4.md) , pro2 , meta и любые будущие. Каждый участник хранит свои данные в своём native формате (так, как удобно автору). Единственное требование для участия — положить два файла в корень:
- [nautilus](../05-habr-projects/memory/memnet.md).json — minimal manifest (имя, формат, версия, мосты)
...
- Наследуется от [BaseAdapter](01-интегральный-анализ-профиля-svend4.md)
- Умеет fetch(query) — поиск в своём native формате
- Умеет describe() — возврат метаданных
- Конвертирует внутренние данные в унифицированный [PortalEntry](01-интегральный-анализ-профиля-svend4.md)
- Знает свои bridges — как его концепты мапятся на концепты других репо
Адаптеры — это граница между мирами . Внутри — любой формат. Снаружи — стандартный [PortalEntry](01-интегральный-анализ-профиля-svend4.md) . Это классический Adapter Pattern (GoF), применённый к data federation.
#### Слой 3 — Движок портала
portal.py — центральная оркестрация. Отвечает за:
- Загрузку [nautilus](../05-habr-projects/memory/memnet.md).json и регистрацию адаптеров
...
Движок не знает ничего про native форматы — он знает только про BaseAdapter и [PortalEntry](01-интегральный-анализ-профиля-svend4.md) . Это обеспечивает extensibility: добавить новый репо — значит написать один новый адаптер, не трогая движок.
#### Слой 4 — Интерфейсы доступа
Три канала входа к движку:
- CLI — portal.py --query "кристалл" для прямой работы из shell
- HTTP/Web — portal.py --serve , статический index.html на [GitHub](../01-svyazi/03-component-catalog.md) Pages для браузерного доступа
...
Принцип локальной автономии. Каждый репо в экосистеме остаётся самодостаточным . Он работает без Nautilus. Nautilus — это опциональный федерационный слой сверху, не требование. Если Nautilus завтра исчезнет — [info1](01-интегральный-анализ-профиля-svend4.md), pro2, meta продолжают жить как обычные git-репо.
Принцип минимального манифеста. Чтобы подключить репо к Nautilus — нужен только [nautilus](../05-habr-projects/memory/memnet.md).json (10–20 строк JSON) и passport.md (1 страница markdown). Это всё. Никаких форматных требований к остальному содержимому.
...
- Чёткие определения nautilus.json schema, passport.md format, [BaseAdapter](01-интегральный-анализ-профиля-svend4.md) interface
...
Deliverables:
- BaseAdapter + [PortalEntry](01-интегральный-анализ-профиля-svend4.md) + Consensus + QueryResult классы
- portal.py — движок с CLI-интерфейсом
- Рабочие адаптеры для [info1](01-интегральный-анализ-профиля-svend4.md), pro2, meta
- [nautilus](../05-habr-projects/memory/memnet.md).json и passports/*.md для текущей экосистемы
...
Deliverables:
- index.html — [GitHub](../01-svyazi/03-component-catalog.md) Pages портал (static)
- Fetch из [nautilus](../05-habr-projects/memory/memnet.md).json в браузере, минимальный UI для query
- README.md с рабочими ссылками, quick start, example queries
- Исправление broken links из embedded-версии
Smoke test: открыть [GitHub](../01-svyazi/03-component-catalog.md) Pages URL, выполнить query через браузер, получить отформатированный результат.
...
- PyPI package [nautilus](../05-habr-projects/memory/memnet.md)-portal с CLI-entry-point
...
Адаптеры не импортируют друг друга. Если нужен bridge между info1 и pro2 — он описан декларативно в [nautilus](../05-habr-projects/memory/memnet.md).json поле bridges , и интерпретируется движком, а не hardcoded в адаптере.
[BaseAdapter](01-интегральный-анализ-профиля-svend4.md) — только абстрактный контракт. В нём не должно быть логики, только сигнатуры методов + docstrings. Изменения в BaseAdapter = breaking change = bump major version протокола.
Версионируемые схемы JSON. [nautilus](../05-habr-projects/memory/memnet.md).json включает поле protocol_version . Движок поддерживает несколько версий через shim-логику.
...
Добавление нового репо — это PR, не hardcode. Когда кто-то хочет подключить новый репо, он открывает PR с: новым entry в [nautilus](../05-habr-projects/memory/memnet.md).json , новым adapters/<name>.py , новым passports/<name>.md . Ревью по чеклисту, merge.
...
Nautilus не читает приватные репо через MCP по умолчанию. Адаптеры для приватных репо (как ваши soz* ) должны быть отдельно isolated — либо жить в отдельном private-[nautilus](../05-habr-projects/memory/memnet.md)/ репо, либо через явный opt-in в конфиге. Это защита на случай, если MCP-server будет подключён к агенту, которому не следует иметь доступ к чувствительным данным.
passports/ репо — публичны, но содержат только метаданные , не данные. В паспорте [info1](01-интегральный-анализ-профиля-svend4.md) можно написать «74 документа с α-уровнями про такие-то темы», но не сами документы.
...
Шаг 3 (параллельно, 2 часа). Исправить README.md в standalone-репо: вернуть туда архитектурный контекст из embedded-версии (то, что было в [pro2](01-интегральный-анализ-профиля-svend4.md)/nautilus/README.md), убрать broken links, добавить правильный Quick Start для нового URL.
...
- Три шаблона passport.md — один для [info1](01-интегральный-анализ-профиля-svend4.md), один для pro2, один для meta. Каркас, в который вы подставите детали.
