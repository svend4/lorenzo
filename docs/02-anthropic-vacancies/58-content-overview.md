---
state: approved
---

# Content Overview

<!-- toc-auto -->
## Contents

- [Content Overview](#content-overview)
- [Использование](#использование)
- [Смотрите также](#смотрите-также)
- [Похожие документы](#похожие-документы)


> [!TIP]
> Документ содержит практические рекомендации и лучшие практики.

<!-- alert-added -->

<!-- summary -->
> Документ содержит практические рекомендации и лучшие практики. Мост-документация — формальное описание связи между двумя 56-essence (сходство 0.13) 12-content-overview (сходство 0.12)
   системами
 --
 --
Использование
 
Смотрите также
 56-essence
 --
Похожие документы
 56-essence (сходство 0.13)
 12-content-overview (сходство 0.12)
 60-bridges (сходство 0.12)
Документ инд

---
<!-- tags: anthropic, rag -->

> [!TIP]
> Обзорный документ. Рекомендуется читать перед погружением в детали.





## Content Overview

**Объём:** 
- 64 гексаграммы с расширенными описаниями
- 256 правил CA с полной dynamics analysis
- [? количество cross-mappings между ними]

**Основные категории:**

1. **Гексаграммные записи** — каждая из 64 гексаграмм с:
   - Классическим именем (King Wen)
   - Бинарным представлением (6 линий)
   - Символической интерпретацией из И-Цзин
   - Ассоциированными CA-правилами

2. **CA-правила** — каждое из 256 правил с:
   - Rule number (0–255)
   - Wolfram class (I stable / II periodic / III chaotic / IV complex)
   - Simulation results (патерны, attractors)
   - Cross-link к гексаграмме(ам)

3. **Мост-документация** — формальное описание связи между двумя 
   системами

---

<!-- see-also -->

---

## Использование

```bash
# Поиск по теме документа
python scripts/improve_semantic_search.py --query "Content Overview"
```

## Смотрите также
- [56-essence](56-essence.md)


<!-- similar-docs -->

---

## Похожие документы
- [56-essence](56-essence.md) (сходство 0.13)
- [12-content-overview](12-content-overview.md) (сходство 0.12)
- [60-bridges](60-bridges.md) (сходство 0.12)

Документ индексирован в базе знаний репозитория. Навигация возможна через семантический поиск и граф концептов репозитория Lorenzo. Материал доступен для семантического поиска, BM25-поиска и навигации через граф концептов. Используйте скрипты группы reports для получения актуальной статистики по разделу. Рекомендуется начинать с основных документов раздела и переходить к деталям через внутренние ссылки. Все связанные документы доступны через граф концептов и поисковый индекс репозитория Lorenzo. Документы раздела индексированы в поисковой базе и доступны для семантического поиска и BM25. Для автоматического обновления раздела используйте инструменты из группы scripts improve_run_all. Документ индексирован в базе знаний репозитория. Навигация возможна через семантический поиск и граф концептов репозитория Lorenzo. Материал доступен для семантического поиска, BM25-поиска и навигации через граф концептов.

<!-- backlinks -->

---

**Кто ссылается на этот документ (8):**
- [38-content-overview](38-content-overview.md)
- [README](README.md)
- [CONCEPTS](../CONCEPTS.md)
- [OUTLINE](../OUTLINE.md)
- [READABILITY](../READABILITY.md)
- [READING_TIME](../READING_TIME.md)
- [SEARCH](../SEARCH.md)
- [TABLES](../TABLES.md)

