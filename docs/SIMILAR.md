# Похожие документы

**Файлов проанализировано:** 496

Для каждого документа — топ-3 похожих по словарному пересечению (Jaccard).

## Топ-20 самых похожих пар

| Сходство | Файл A | Файл B |
|----------|--------|--------|
| 1.000 | `273-infrastructure-for-ai-collaborative-intellectual-w.md` | `151-open-knowledge-work-foundation-md.md` |
| 0.965 | `03-карта-найденных-проектов-и-паттернов.md` | `03-component-catalog.md` |
| 0.957 | `09-архитектурные-зазоры-которые-важнее-новых-инструме.md` | `09-architectural-gaps.md` |
| 0.955 | `05-план-прототипа-и-возможные-контакты.md` | `07-mvp-planning.md` |
| 0.954 | `06-безопасность-приватность-и-бюджетный-роутинг.md` | `06-security-privacy.md` |
| 0.948 | `13-контактная-стратегия-и-узкие-вопросы-для-авторов.md` | `13-contacts.md` |
| 0.945 | `12-дорожная-карта-прототипа-следующей-итерации.md` | `12-roadmap.md` |
| 0.939 | `11-интеграционный-контракт-который-стоит-зафиксироват.md` | `11-integration-contracts.md` |
| 0.927 | `94-19-adr-001-federation-over-merging.md` | `26-14-adr-001-federation-over-merging.md` |
| 0.905 | `zodigancode.md` | `cutcode.md` |
| 0.905 | `mixaill76.md` | `dmitriila.md` |
| 0.901 | `07-выводы.md` | `08-conclusions.md` |
| 0.881 | `zodigancode.md` | `vladspace.md` |
| 0.881 | `vladspace.md` | `tagir-analyzes.md` |
| 0.881 | `vladspace.md` | `mixaill76.md` |
| 0.881 | `dmitriila.md` | `vladspace.md` |
| 0.881 | `cutcode.md` | `vladspace.md` |
| 0.881 | `251-ai-support-through-configurable-specialist-ensembl.md` | `209-a-typology-of-ai-agents-on-the-principal-side-and-.md` |
| 0.875 | `04-приоритетные-ансамбли.md` | `04-ensembles-overview.md` |
| 0.874 | `10-новые-ансамбли-следующего-шага.md` | `10-second-order-ensembles.md` |

## По разделам


### 02-anthropic-vacancies

- `273-infrastructure-for-ai-collaborative-intellectual-w.md` ↔ `151-open-knowledge-work-foundation-md.md` (1.000)
- `94-19-adr-001-federation-over-merging.md` ↔ `26-14-adr-001-federation-over-merging.md` (0.927)
- `251-ai-support-through-configurable-specialist-ensembl.md` ↔ `209-a-typology-of-ai-agents-on-the-principal-side-and-.md` (0.881)
- `187-слой-представительских-агентов-md.md` ↔ `166-representative-agent-layer-md.md` (0.846)
- `344-системный-промпт-для-lorenzo-project.md` ↔ `249-composite-skills-agent-md.md` (0.800)

### 04-ai-collaborations

- `03-карта-найденных-проектов-и-паттернов.md` ↔ `03-component-catalog.md` (0.965)
- `09-архитектурные-зазоры-которые-важнее-новых-инструме.md` ↔ `09-architectural-gaps.md` (0.957)
- `05-план-прототипа-и-возможные-контакты.md` ↔ `07-mvp-planning.md` (0.955)
- `06-безопасность-приватность-и-бюджетный-роутинг.md` ↔ `06-security-privacy.md` (0.954)
- `13-контактная-стратегия-и-узкие-вопросы-для-авторов.md` ↔ `13-contacts.md` (0.948)

### 05-habr-projects

- `memnet.md` ↔ `00-intro.md` (0.669)

### contacts

- `zodigancode.md` ↔ `cutcode.md` (0.905)
- `mixaill76.md` ↔ `dmitriila.md` (0.905)
- `zodigancode.md` ↔ `vladspace.md` (0.881)
- `vladspace.md` ↔ `tagir-analyzes.md` (0.881)
- `vladspace.md` ↔ `mixaill76.md` (0.881)

### root

- `QA.md` ↔ `QA.md` (0.837)
- `SEARCH.md` ↔ `LANGUAGE_STATS.md` (0.782)
- `SEARCH.md` ↔ `READING_ORDER.md` (0.767)
- `READING_ORDER.md` ↔ `README.md` (0.728)
- `SEARCH.md` ↔ `README.md` (0.723)

### templates

- `contact-outreach.md` ↔ `nlaik.md` (0.600)
- `contact-outreach.md` ↔ `sonia-black.md` (0.579)
- `contact-outreach.md` ↔ `vladspace.md` (0.574)
