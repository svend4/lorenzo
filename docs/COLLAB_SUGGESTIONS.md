...
<!-- summary -->
> Автоматический поиск партнёрских проектов для: «review локальное ревью кода lorenzo имеет добавляет самих скриптов scripts impro»
> **Дата:** 2026-05-12 08:29  **Кандидатов:** 1

---

<!-- tags: collaboration, projects, recommendations, svyazi -->

**Источник:** `docs/06-discovery/round-15/projects/ai-review-local-llm.md`

**Запрос:** review локальное ревью кода lorenzo имеет добавляет самих скриптов scripts improve каждом просто синтаксис проверка промптов логики соответствия openlight принципу

---

## 1. Svyazi

**Релевантность:** `0.431`  **Тип:** `project`  **Состояние:** `raw`  **Связей:** 8
...
```
**Кому:** kksudo (@kksudo)
**Тема:** Коллаборация по теме «review локальное ревью кода lorenzo имеет добавляет самих ск»
...
5. Повторить поиск с уточнённым запросом:
   ```
   python scripts/improve_collab_finder.py --query "review локальное ревью кода lorenzo имеет добавляе"
   ```

_Сгенерировано: 2026-05-12 08:29  |  Алгоритм: TF-IDF + BM25 + граф_
