...
<!-- summary -->
> Автоматический поиск партнёрских проектов для: «dspy программирование языковых моделей вместо промптинга lorenzo использует impr»
> **Дата:** 2026-05-12 08:21  **Кандидатов:** 4

---

<!-- tags: collaboration, projects, recommendations, svyazi -->

**Источник:** `docs/06-discovery/round-14/projects/dspy-prompt-optimizer.md`

**Запрос:** dspy программирование языковых моделей вместо промптинга lorenzo использует improve enrich фиксированными промптами промпт обогащения карточек оптимизируется автоматически метрике полнота секций качество summary релевантность тегов пайплайн лучшие ответы токенах
...
```
**Кому:** kksudo (@kksudo)
**Тема:** Коллаборация по теме «dspy программирование языковых моделей вместо промптинга lor»
...
**Релевантность:** `0.297`  **Тип:** `project`  **Состояние:** `raw`  **Связей:** 7
...
**Релевантность:** `0.182`  **Тип:** `project`  **Состояние:** `raw`  **Связей:** 8
...
**Релевантность:** `0.146`  **Тип:** `project`  **Состояние:** `raw`  **Связей:** 7
...
5. Повторить поиск с уточнённым запросом:
   ```
   python scripts/improve_collab_finder.py --query "dspy программирование языковых моделей вместо пром"
   ```

_Сгенерировано: 2026-05-12 08:21  |  Алгоритм: TF-IDF + BM25 + граф_
