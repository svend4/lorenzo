...
- прочитать в первую очередь. **Проекты:** Svyazi, CardIndex, AgentFS, knowledge-space, mclaude, AI Factory, Rufler, LiteParse --- <!-- toc --> ## Содержание - [📊 Диаграммы Mermaid (8)](#диаграммы-me  
  _→ CODE_BLOCKS_

- Svyazi + AgentFS + NGT/Yodoca + LiteParse: даёт уже полезный MVP 2 **B:** `docs/CONCEPTS.md` > md) Первое : Svyazi + AgentFS + NGT/Yodoca + LiteParse: даёт уже полезный MVP → 01-executive-summary(doc  
  _→ CONTRADICTIONS_
...
- добавить AI Factory/mclaude/Rufler/Sequential как build‑ и moderation‑контур 3 **B:** `docs/CONCEPTS.md` > md) Второе : добавить AI Factory/mclaude/Rufler/Sequential как build‑ и moderation‑контур →  
  _→ CONTRADICTIONS_
...
_...ещё 104 записей в этой категории_
