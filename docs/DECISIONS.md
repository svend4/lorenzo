...
Автоматически извлечено из всех документов: **427 записей**
...
- слой — не память, не RAG[^rag] и не оркестр **Проекты:** Svyazi[^svyazi], CardIndex[^cardindex], AgentFS[^agentfs], mclaude, AI Factory, Rufler[^rufler], LiteParse, Yodoca[^yodoca] --- <!-- tags: mem  
  _→ 08-conclusions_
...
- Svyazi + AgentFS + NGT^ngt/Yodoca + LiteParse: это даёт уже полезный MVP. > 🏷️ **Ключевые слова:** `svyazi`, `summary`, `executive`, `collaborations`, `продолжение`, `conclusions`, `проект`, `first` >  
  _→ 07-выводы_
...
- прочитать в первую очередь. <!--… _[→ Читать полностью](05-habr-projects/memory/yodoca.md)_ --- ## Глава 11: NGT — граф памяти > <!-- abstract-auto --> > **Абстракт** (авто) > > 🎯 **Проблема:** У  
  _→ NARRATIVE_
...
- в секции: 44 слов, 1 файлов** ## 📁 Contacts (`docs/contacts/`) ### [contacts](README.md) > Файлов: 14 - Содержание _Слов: 90_ ### [Контакт: AnastasiyaW / knowledge-space, mclaude](contacts/ana  
  _→ OUTLINE_
...
- создать документы](#рекомендуется-создать-документы) - [Детали по топ-20 пробелам](#детали-по-топ-20-пробелам) - [`LiteParse` (50 файлов)](#liteparse-50-файлов) - [`BSL` (37 файлов)](#bsl-37-файло  
  _→ CONTENT_GAPS_

- создать документы | Концепция | Упоминаний | Рекомендуемая папка | |-----------|-----------|-------------------| | `LiteParse` | 50 | `docs/04-ai-collaborations/` | | `BSL` | 37 | `docs/04-ai-collabo  
  _→ CONTENT_GAPS_
...
- склеивать](#ограничения-лицензии-и-что-пока-лучше-не-склеивать) - [Упоминается в](#упоминается-в) - [Связанные документы](#связанные-документы) ---  
  _→ 14-limitations_
...
_...ещё 287 записей в этой категории_
...
- [06 Security Privacy](01-svyazi/06-security-privacy.md) _17%_
- [07 Mvp Planning](01-svyazi/07-mvp-planning.md) _17%_
- [08 Conclusions](01-svyazi/08-conclusions.md) _17%_
- [12 Roadmap](01-svyazi/12-roadmap.md) _17%_
- [Executive summary](04-ai-collaborations/01-executive-summary.md) _17%_
