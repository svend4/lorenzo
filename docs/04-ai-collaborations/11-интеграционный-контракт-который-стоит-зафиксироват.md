...
> 🔧 **Подход:** Любой retrieval‑ответ, match suggestion, profile enrichment или auto‑summary должен возвращать не только текст, но и sourceid , page , span , box , retrievalmethod , confidence , s
> ✅ **Результат:** Это не “идеальная онтология”, а минимальный договор, который позволяет системам вообще разговаривать между собой.
> 🏷️ **Ключевые слова:** `svyazi`, `memory`, `контракт`, `через`, `search`, `проект`, `который`, `agentfs`
>
...
**Проекты:** Svyazi[^svyazi], CardIndex[^cardindex], AgentFS[^agentfs], mclaude, AI Factory, [LiteParse](../01-svyazi/01-executive-summary.md), Legal RAG[^rag], Hybrid RAG
...
, список source links, список relation edges, временные метки и хэш полезной нагрузки. Эта структура логически следует из [CardIndex](../01-svyazi/01-executive-summary.md)‑мышления Svyazi, immutable/event‑style практик AgentFS и Memory OS, а также из необходимости разводить truth и proposal в memory‑системах. Это не “идеальная онтология”, а минимальный договор, который позволяет системам вообще разговаривать между собой. citeturn41search0turn27view0turn39view3turn20view16
...
,
supporting_nodes
. Для документов это page+box; для чатов и заметок — message/thread/time span; для голосовых эпизодов — timestamp window; для ассоциативных выводов — список triggered nodes и path explanation. Это прямой синтез из [LiteParse](../01-svyazi/01-executive-summary.md)/research-docs, Legal RAG, Hybrid RAG и Graph RAG. Без такого формата нельзя построить ни нормальную ручную модерацию, ни “объяснение рекомендации”. citeturn20view5turn20view6turn34view2turn34view3
...
Четвёртый контракт — **Skill and Tool Policy**. Каждый skill или MCP[^mcp]‑инструмент должен иметь класс доступа, класс среды, условия вызова и postcondition. Простейшее разбиение:
read
,
annotate
,
...
## Похожие документы
- [11-integration-contracts](../01-svyazi/11-integration-contracts.md) (сходство 1.00)
- [366-технический-stack-[svyazi](../01-svyazi/00-intro-part2.md)-2-0-foundation](docs/02-anthropic-vacancies/366-технический-stack-svyazi-2-0-foundation.md) (сходство 0.19)
- [09-архитектурные-зазоры-которые-важнее-новых-инструме](09-архитектурные-зазоры-которые-важнее-новых-инструме.md) (сходство 0.16)
...
python scripts/improve_semantic_search.py --query "Интеграционный контракт который стоит"
```

## Смотрите также
- [11-integration-contracts](../01-svyazi/11-integration-contracts.md)
- [366-технический-stack-[svyazi](../01-svyazi/00-intro-part2.md)-2-0-foundation](docs/02-anthropic-vacancies/366-технический-stack-svyazi-2-0-foundation.md)
- [09-architectural-gaps](../01-svyazi/09-architectural-gaps.md)
- [09-архитектурные-зазоры-которые-важнее-новых-инструме](09-архитектурные-зазоры-которые-важнее-новых-инструме.md)
