...
> 🏷️ **Ключевые слова:** `search`, `svyazi`, `knowledge`, `first`, `проект`, `agentfs`, `space`, `rufler`
>
...
<!-- summary -->
> Ниже — не все теоретически возможные комбинации, а **пять ансамблей с максимальным приростом свойств при минимальном интеграционном риске**. 🎯 Проблема: citeturn35search0 - Контекст реального мира доступен агенту как tool, а не как догадка: Self‑Aware MCP закрывает проблемы часового пояса, ОС, даты и локации.
**Проекты:** Svyazi[^svyazi], CardIndex[^cardindex], AgentFS[^agentfs], [knowledge-space](../01-svyazi/03-component-catalog.md)[^knowledge-space], mclaude, AI Factory, Rufler[^rufler], LiteParse
...
Это базовый сценарий для Svyazi‑2.0: Svyazi отвечает за извлечение и нормализацию профилей, AgentFS — за единое файловое ядро и политику, [knowledge-space](../01-svyazi/03-component-catalog.md) — за agent‑readable reference layer, NGT[^ngt] Memory — за быстрые ассоциативные связи, Yodoca[^yodoca] — за ночную консолидацию и забывание шумов. Такой стек превращает “случайные находки коллабораций” в воспроизводимую машинерию. citeturn41search0turn27view0turn33view2turn22view4turn21view0
mermaid
flowchart LR
...
- **Agent‑first knowledge retrieval**: [knowledge-space](../01-svyazi/03-component-catalog.md) снижает стоимость «ориентации в проекте», потому что хранит не туториалы, а уже очищенные reference‑карты с граблями и рабочими паттернами. citeturn33view3turn37search1
...
- **Улучшение не по интуиции, а по циклу “изменил → измерил → откатил/сохранил”**: [AutoResearch](../01-svyazi/01-executive-summary.md) ровно эту петлю и формализует. citeturn20view19
...
mermaid
flowchart LR
    A[Микрофон / встреча / голосовая заметка] --> B[Whisper / Handy / [OpenWhispr](../01-svyazi/03-component-catalog.md) / GigaAM]
...
## Похожие документы
- [04-ensembles-overview](../01-svyazi/04-ensembles-overview.md) (сходство 0.92)
- [03-карта-найденных-проектов-и-паттернов](03-карта-найденных-проектов-и-паттернов.md) (сходство 0.17)
- [03-component-catalog](../01-svyazi/03-component-catalog.md) (сходство 0.16)
...
python scripts/improve_semantic_search.py --query "Приоритетные ансамбли"
```

## Смотрите также
- [04-ensembles-overview](../01-svyazi/04-ensembles-overview.md)
- [03-карта-найденных-проектов-и-паттернов](03-карта-найденных-проектов-и-паттернов.md)
