...
> 🏷️ **Ключевые слова:** `svyazi`, `search`, `memory`, `только`, `privacy`, `security`, `нужен`, `модель`
>
...
python scripts/improve_semantic_search.py --query "Безопасность приватность и бюджетный"
```
