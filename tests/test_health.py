...
def test_main_creates_health_md(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "DOCS", tmp_path)
...
def test_main_health_has_content(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "DOCS", tmp_path)
    monkeypatch.setattr(mod, "ROOT", tmp_path)
    (tmp_path / "doc.md").write_text("# Title\n\nContent here.", encoding="utf-8")
    mod.main()
    text = (tmp_path / "HEALTH.md").read_text(encoding="utf-8")
    assert "# " in text
...
def test_main_health_starts_with_heading(tmp_path, monkeypatch):
    monkeypatch.setattr(mod, "DOCS", tmp_path)
    monkeypatch.setattr(mod, "ROOT", tmp_path)
    mod.main()
    text = (tmp_path / "HEALTH.md").read_text(encoding="utf-8")
