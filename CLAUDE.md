...
scripts/
  ...
  improve_export_report.py   — единый сводный отчёт (Executive Summary) → REPORT.md
  ...
  improve_cross_section.py  — граф концептов между секциями → CROSS_SECTION.md
  improve_digest_auto.py    — автодайджест изменений за N дней → DIGEST_AUTO.md
  ...
  # Расширенный NLP-анализ (группа nlpplus)
  improve_textrank.py          — TextRank резюме каждого файла → SUMMARIES.md
  improve_heading_audit.py     — аудит иерархии заголовков (пропуски, дубли, пустые)
  improve_language_split.py    — RU/EN состав → LANGUAGE_STATS.md; --split разделяет файлы
  improve_question_extractor.py — вопросы/гипотезы/TODO/открытые темы → QUESTIONS.md
  improve_passive_voice.py     — пассивный залог, номинализации, канцеляризмы
...
python scripts/improve_cross_section.py                # граф концептов между секциями
python scripts/improve_cross_section.py --top 30 --format dot  # + Graphviz
python scripts/improve_digest_auto.py                  # дайджест за 7 дней
python scripts/improve_digest_auto.py --days 30        # за месяц
python scripts/improve_digest_auto.py --since 2026-04-01  # с конкретной даты
python scripts/improve_export_report.py                # сводный отчёт → REPORT.md
python scripts/improve_export_report.py --title "Svyazi — Brief"
```
...
### Расширенный NLP-анализ (nlpplus)
```bash
# TextRank резюме
python scripts/improve_textrank.py                                # SUMMARIES.md (291 файл)
python scripts/improve_textrank.py --apply                        # вставить резюме в файлы
python scripts/improve_textrank.py --query "агент память"         # резюме по теме
python scripts/improve_textrank.py --sentences 5                  # 5 предложений

# Структура документов
python scripts/improve_heading_audit.py                           # HEADING_AUDIT.md
python scripts/improve_heading_audit.py --verbose --section 01-svyazi
python scripts/improve_empty_sections.py                          # EMPTY_SECTIONS.md
python scripts/improve_empty_sections.py --fill --section 05-habr-projects

# Язык и стиль
python scripts/improve_language_split.py                          # LANGUAGE_STATS.md
python scripts/improve_language_split.py --split                  # разделить MIX-файлы
python scripts/improve_passive_voice.py                           # PASSIVE_VOICE.md
python scripts/improve_passive_voice.py --verbose --section 01-svyazi

# Вопросы и темы
python scripts/improve_question_extractor.py                      # QUESTIONS.md
python scripts/improve_question_extractor.py --type open          # только открытые
python scripts/improve_question_extractor.py --type todo
